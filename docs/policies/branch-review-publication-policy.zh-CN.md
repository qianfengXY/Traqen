...
- 影响；
- 建议修改和可观察的验收条件；
- 确认该 Finding 的 reviewer 身份；
- 提交模型身份；以及
- 尚未解决的分歧或开放问题（如有）。

Issue 标题和正文只使用简体中文，不得追加英文标题、英文摘要或单独的英文段落。代码符号、路径、
命令、标识符和引用的源文本在翻译会导致证据失真时保持原样。

每条 Issue 必须包含稳定的模型身份行，例如 `提交模型：CodeX / GPT-5`，并在正文末尾带上提交猫猫的
猫猫模型签名，例如 `[CodeX/GPT-5🐾]`。提交模型身份与确认该 Finding 的 reviewer 身份是两个不同
字段：前者说明谁发布或最后一次实质改写了 Issue，后者记录哪些独立证据满足了共识门。

既有 Issue 纳入本政策时，应按上述规则统一标题和正文，但不得删除历史评论或其他审计证据。
...
- 证据无法复现或定位；
- reviewer 未形成实质共识；
- 必需的双语独立 Review 或收敛记录缺失或含义不等价；
- Issue 标题或正文不是仅简体中文，或缺少提交模型身份、猫猫模型签名；或
