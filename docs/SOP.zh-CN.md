...
| 6. 独立 Review | 按 Cat Café 风险路由选择一位非作者独立 reviewer，覆盖同一准确 SHA，并把仅 Review 记录留在 Git 之外；只有明确要发布 Issue 的候选 Finding 才进入发布政策的双独立共识门 | 审阅来源、准确 SHA、verdict 与证据；发布 Issue 时另附双独立收敛 |
| 7. 合入和验收 | 解决阻断发现、通过合入门后，将已审本地分支合入本地 `main`；在本地 `main` 验收后仅推送 `origin/main`，再清理已合入分支及其 worktree | 本地合入回执、验收证据、`main` 同步证据与清理结果 |

## 本地合入与远端同步

- 本地 `main` 是唯一合入真相源。侧分支及其 worktree 仅用于隔离实施、Review 和验收，不能推送到远端，也不能通过远端分支或远端 PR 绕过本地 `main`。
- 审阅和门禁覆盖侧分支的准确 SHA 后，作者将该分支合入本地 `main`；合入后的检查与验收从本地 `main` 运行。
- 只有本地 `main` 验收通过后，才允许推送 `origin/main`。远端仅保留 `main` 分支。
- 远端 `main` 与本地 `main` 对账为同一提交后，立即删除已合入的本地分支并移除对应 worktree；脏工作区或运行占用必须先保全并报告，禁止强制清理。
...
- 禁止同一人自审。普通分支 Review 不因请求本身触发双 Review；只有明确要发布为 Issue 的候选 Finding 才触发更严格的双 Review 发布政策。
