...
1. creates an immutable Source/Build/Deployment/Runtime Snapshot Manifest;
2. scans the reference repository into 49 locatable Fact nodes and 100 edges;
...
9. ingests Runner-signed HTTP, database, assertion, and lifecycle Evidence, including the normalized trusted catalog SQL, query reference, redacted parameters, and returned rows;
10. persists a complete Feature trace chain with zero gaps.

## Change and repair

The pilot copies the target to an isolated directory and makes a real source change to the endpoint handler. The generic Scanner observes the new Snapshot, and the generic Fact comparison produces a ChangeSet with the submit-order Feature affected.
...
`TestSpec.sourceSnapshotId` records where the test protocol was generated; it is not the deployment that every future regression must execute. The signed Runner task binds the exact execution Snapshot, and the resulting Evidence binds Snapshot Manifest, deployment, TestSpec version, and Runner. Feature traceability only becomes complete when current-Snapshot conformance and current-deployment Evidence both exist.
...
- four reference-target tests pass;
- the automated vertical-pilot regression passes;
- the pilot reports two Reverse Skills, two candidate sources, `PASS` on both deployments, preserved `CONFIRMED` authority during change, `CONFORMS` after repair, rejection of historical Evidence for the new deployment, and a final complete chain with zero gaps.
