...
updated: 2026-09-16
version: "1.0"
baseline_id: F002-functional-design-v1.0
status: functional-design-final
---

# F002 确定性事实层：功能设计基线 V1.0

## 1. 定稿范围

根据 co-creator 于 **2026-09-16 03:40 UTC** 的确认：“将本次F002设计文档，发布吧，作为定稿基线”，本页正式承接已核对的功能文字与 **功能全景 v4 · F003 风格版**，成为 F002 当前功能设计真相源。

- **已定稿：** 目标与边界、六组 24 项子功能、五部分逻辑交付物、编号原则、F003 双向取证回路及四种返回、事实成立与质量约束。
- **保持待定：** 第 9 节的原文读取接口、具体 Schema、身份算法、首期提取能力、检查点协议、物理存储和量化验收值；发布不替代这些决策，也不修改 ADR。
- **确认依据：** Kimi 于 2026-09-15 08:22 UTC 独立核对风格版图与文字，结论为功能、产出、回路和边界无漂移；本次复用该图文核对，只做发布编排、入口更新和校验。
- **状态边界：** 这是功能设计定稿，不是实现完成、测试通过或实施启动授权。Feature 状态保留 `Spec`，未勾选历史实现验收项。
- **版本区分：** 文档基线为 V1.0，沿用的图稿版本为 v4；两者不是同一个版本序列。简体中文为规范文本，历史讨论及英文参考不与本页竞争当前真相源。
...
![F002 功能全景 v4 F003 风格版：六组二十四项子功能、受控材料视图、F003 主导调查、双向取证及待定接口](assets/f002-functional-panorama-v4-f003-style.zh-CN.png)

[打开风格优化后的大图](assets/f002-functional-panorama-v4-f003-style.zh-CN.png) · [原始生成记录与提示词](../../../feature-discussions/2026-09-09-F002-deterministic-facts-design/assets/f002-functional-panorama-v4-f003-style.prompt.md) · [历史图稿与修订来源](../../../feature-discussions/2026-09-09-F002-deterministic-facts-design/README.zh-CN.md)

本次原样发布已核对的图片，没有重绘、改字、改线或重新生成。图内“讨论稿 · 参照 F003 全景 V1.0”保留为绘制时的历史标识；**本页第 1 节的正式确认决定当前基线身份**，不为修改状态标签而改变已核对产物。F003 当前功能设计为 [V2.0](../F003-traceability-graph/README.md)，也是本图的视觉风格参考，不据此关闭跨功能接口待定项。

- **紫色请求：** F003 → F002，定向取证／疑似漏提反馈。
- **蓝色返回：** F002 → F003，处理结果、依据及适用版本／范围。
- **青色输出：** F002 提供按范围读取的参考结果和材料入口，不是 F003 的唯一分析材料。
- **琥珀色虚线及角标：** 原文读取接口待联合对齐，非已批准的直连；现行 ADR-0003 准入边界仍有效。
- **封存含义：** 范围内全终态可有明确 Gap，待处理不能伪装终态；封存不证明提取完整。
- **编号含义：** 功能编号、对象编号和事实编号相互区分；图内短号仅为说明样例。

### 历史措辞的纠正与持续适用的边界

下表用于解释旧图措辞，不把旧图中已纠正的断言重新作为 v4 规则。

| 图中表述或容易产生的理解 | 本基线持续适用的补充与限定 |
|---|---|
...
图由 AI 辅助绘制，与产品中 F002 的事实提取机制无关；F002 事实提取仍不依赖 AI。

## 4. 六项功能、二十四项子功能

下表与 v4 的六组 24 项子功能对应，不是实现完成清单。用户可按对象、事实、证据、缺口和版本浏览；API 目录或树只是其中一种投影。独立正确性验证横贯提取、关联与交付，具体边界见第 7 节，不单列第七组功能。
...
- 本基线的完整事实引用为 `workspaceId + graphVersionId + factId`；其中 `graphVersionId` 指 F002 事实数据集版本，不与 F003 业务图谱版本混用。
...
本基线的推进方向是：**基础普查 + F003 问题驱动的定向取证 + 独立正确性验证。** 不要求先提完全部事实再启动业务调查，也不将提取能力缩成“Agent 问到什么才登记什么”。

### 定向取证／疑似漏提反馈（v4 回路）

**请求方向：F003 → F002。** 请求给出定向取证问题或疑似漏提及材料位置，绑定 Workspace、来源版本与范围，预算与停止条件有界。Agent 的怀疑是待核查输入，不是修改事实的指令。

**返回方向：F002 → F003。** 返回处理结果、依据以及适用版本／范围，区分四种结果：

| 处理结果 | 返回与约束 |
|---|---|
| 已有依据 | 返回已有事实与证据。 |
| 补提／纠正确认 | 独立验证后发布新版本；旧版本和引用保留。 |
| 仍不确定／不支持 | 返回已发现限制与 Gap，不伪造确定结论。 |
| 越权／版本不符 | 拒绝并说明原因，不扩大授权或混用版本。 |

F003 是重要的应用验证渠道，但不是 F002 唯一的验收者。[F003 当前功能设计](../F003-traceability-graph/README.md) 采用“自动收录 Agent 分析态 + 例外人工审核”；本基线不恢复旧版“所有候选都必须先人工批准”的规定。
...
本版确认的是功能设计基线，不据此冻结技术栈 MVP。此前 Java/Spring、Node.js/Express、OpenAPI 的讨论是首批样本与能力清单的参考；应同时结合实际材料清单和代表性调查需求确认。
...
- 本次定稿不代表功能已实现，不改变来源准入/访问权限，也不自动启动实现。

## 9. 待确认、待对齐项

| 问题 | 当前边界 |
|---|---|
| F001 原文到 F003 的具体读取入口 | [ADR-0003 决策 8](../../decisions/ADR-0003-source-truth-boundary.zh-CN.md) 仍要求 F002 为唯一直接来源包消费者；F003 V1.0 确认原材料主分析，但未定直接读取还是受控服务。本基线要求提供 F002 受控材料视图；读取入口、完整清单交接、鉴权复验、版本绑定需要联合对齐，不在此擅改 ADR。 |
| 出口契约字段与词义 | 五部分内容和完整引用原则由本基线确认；具体 schema、材料定位格式、类型词表、查询/分页与错误返回未定。 |
...
下一轮可先细化出口契约，用代表性 F003 调查和独立参考材料两条线验证，再确定首批实现能力。两边是否并行开发属于实施计划选择，本基线不自动授权该计划。

## 10. 发布依据与历史追溯

| 项目 | 记录 |
|---|---|
| 当前基线 | `F002-functional-design-v1.0`，本页及第 3 节所嵌风格版图。 |
| 讨论线程 | `thread_mtgygk6ew4qqmz7b` |
| 定向取证与返回路径决策 | `0001789004687536-000113-c9b9f8c7` |
| v4 原图独立检查 | Kimi，`0001789455416701-000276-ad9634c8`，2026-09-15 06:56 UTC。 |
| 风格对齐与嵌入授权 | `0001789456971433-000308-709a079d`、`0001789458792213-000336-9a7994b2`。 |
| 风格版图文一致性独立核对 | Kimi，`0001789460522847-000368-40c193ad`，2026-09-15 08:22 UTC；六组 24 项、五部分出口、四种返回、编号、护栏和待定接口逐项对应。 |
| co-creator 定稿及发布授权 | `0001789530041046-000419-df31f98b`，2026-09-16 03:40 UTC。 |
| 采用的图稿 | `assets/f002-functional-panorama-v4-f003-style.zh-CN.png`，1448 × 1086。 |
| 原始生成文件名 | `exec-27631479-1acc-4a6d-b131-c3a490c7fb7e.png` |
| PNG SHA-256 | `ea72b7d8850adb60328588ed4f88a86e6b029867b241246f30be404e03a4e976` |

历史 v1–v4 原图、风格版原件及提示词保留在[讨论来源目录](../../../feature-discussions/2026-09-09-F002-deterministic-facts-design/README.zh-CN.md)，作为决策与生成来源，不再作为独立活动设计。已核对的功能表、出口表、回路与质量边界在本次发布中不变。

相关入口：[F002 Feature 入口与历史 Spec](../../features/F002-feature-api-traceability.zh-CN.md)、[已发布设计索引](../README.md)、[F003 当前设计](../F003-traceability-graph/README.md)、[ADR-0003](../../decisions/ADR-0003-source-truth-boundary.zh-CN.md)。本次只更新 F002 的定稿记录与导航；不宣称 F003、ADR 或实现已随之完成修改。
