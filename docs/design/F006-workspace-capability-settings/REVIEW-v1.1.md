...
design_reference: F005@f4df4bca5de81fe2af86e10db997c18dc3cbdc5d
---
...
> **当前整合状态：图稿已按同一逻辑母版冻结，独立复核须针对同一精确版本完成。** 以下图片位置、文件摘要与检查记录组成待审交付清单；旧图采用的“大屏增加内容”规则已撤回，不能作为本文整体缩放规则的通过证据。
...
最高依据是用户在 F005 的明确纠偏：“14寸与27寸只是缩放的区别，不是要做2套设计，功能、显示、效果这些都一样”（原文坐标见 §11）。以下章节编号对应已发布的 [F005 整体布局与导航设计规范 V1.0](https://github.com/qianfengXY/Traqen/blob/f4df4bca5de81fe2af86e10db997c18dc3cbdc5d/docs/design/F005-layout-navigation/README.md)，逐项落实导航、组件、状态及同一母版规则。该规范经 PR #46 合入；旧V2.1中与用户原话冲突的双尺寸参数不再作为依据。F005已发布不等于本F006稿已获批准，本稿仍待审。
...
| §16 授权、来源与修订记录 | 保留直接原话、F005不可变发布锚点与F006功能来源 | 不把候选frontmatter或伙伴共识当成发布/批准事实 |
...
F006 沿用 F005 的1440×900逻辑母版：主导航224px、顶部上下文栏常态66px、内容外边距32px。顶部上下文栏在长文本/可访问性模式下可自然增高。以下尺寸都是母版逻辑值，不是不同屏幕各自的参数。

团队名单在母版使用固定320px宽（本稿的F006任务布局选择，并非F005统一参数），Main在前、Child依序；编辑区占其余空间。设置次级导航维持同一位置与层级。只读版本摘要按需打开，两种桌面都有同一个入口、同一内容；打开/返回保持选择与滚动，不能只有大屏自动展开。列表、弹窗、长文展开及用户选择的密度也不随设备另设规则。
...
普通窗口任一可用尺寸小于母版（例如1400×860或1280×800）时，整体倍率下限为1，不自动缩小至1以下；使用同一组件和内容重排、展开或滚动，确保名称、错误和全部动作可达。浏览器用户放大导致空间不足时也保留自然可读字号，不反向缩小来抵消用户200%/400%的放大。这是可读性保护，不是第二套设备设计，也不承诺手机和平板。较小窗口/用户放大的检查与正常桌面整体倍率分别记录，不能互相替代。
...
冲突不是另一个产品页面。选中 Agent、其模型和本地勾选与右侧恢复详情保持同源；横幅明确“其他位置更新了草稿，你的编辑仍保留”，同时提供“重试我的草稿”和“采用服务器草稿”。Apply 禁用且不发 activation 请求。

“重试”先重存失败时冻结的 M2，只确认真正发出的 M2；冲突期间更晚的 M3 保持待保存，M2 成功后再补存一次。界面不要求用户理解 M2/M3 编号，但事件证据必须能区分两份内容。横幅存在时的新编辑不会启动后台重试风暴。

“采用服务器草稿”先说明会丢弃未保存编辑，用户明确选择后再替换；成功清理失败态，后续编辑可正常保存。恢复再次失败仍保留内容并显示新原因，不偷偷切到另一策略。未恢复时允许只读查看生效版本或能力范围，但返回团队、同级导航和刷新都保留 M3 与恢复入口，不产生自动写入或 activation，也不会借只读浏览解除冲突。
...
色彩以 F005 V1.0 §6 的语义token为单一来源；排版、间距与控件对应其 §7–§8。下表尺寸均为母版逻辑值，按 §3.2共同缩放，不是另一份独立主题规范。
...
沿用 F005 V1.0 §12 的 WCAG 2.2 AA 设计目标：普通文字对比度至少4.5:1，大文字至少3:1；必要控件边界和非文本图形按该节要求验证。逐一检查瓷白/石墨的默认、选中、焦点、错误、只读和禁用状态，记录适用项与豁免，不能只测主题主文字。此处是设计与后续验收目标，不是已取得完整无障碍认证的声明。
...
| S04 | 同一母版的双尺寸双主题四张主图；去留白、反向缩放后内容与几何一致；320px逻辑名单、授权数和一个带完整作用范围的Apply一致；下方授权及帮助/账号脚可滚动抵达 |
| S05 | 四组能力与来源清晰；本地创建不等于授权，全局失效不能复活 |
| S06 | 两种恢复有不同后果；选中 Agent 与恢复详情一致；M2/M3不误确认；冲突期间经版本/能力/刷新仍零额外自动写与 activation；恢复后才清除阻断态并返回可编辑团队页 |
| S07 | 影响成功/失败/变化、名称确认、取消与焦点返回 |
| S08 | MCP 暂停和历史只读；无伪检测或就绪入口 |
| S09 | 编辑/草稿/Active/Run身份分离；从团队真实入口往返时保留选择和滚动；失败不伪造生效版本 |
| S10 | F003宿主正确，显式标识未接线，不恢复旧入口 |
| 横向约束 | 双主题表单/错误/弹窗；1440/1920/2560正常桌面倍率；1400×860、1280×800与用户放大单独检查；键盘、命中区、浮层定位、无裁切 |
...
- [F005正式规范 V1.0](https://github.com/qianfengXY/Traqen/blob/f4df4bca5de81fe2af86e10db997c18dc3cbdc5d/docs/design/F005-layout-navigation/README.md) `f4df4bca5de81fe2af86e10db997c18dc3cbdc5d`：PR #46于2026-09-19 04:50:43 UTC合入，GitHub状态与正文已独立核实。采用其16章映射、1440母版/224导航/32边距、整体倍率及小窗口下限1。与先前e8d2991候选相比，最终正文仅进一步明确普通小窗口的可读性规则；其他所引章节内容一致。
...
- 冻结图稿源：`ux-v1.0/assets/prototype.html`（SHA-256 `3dd2182219c27a58bd626f0dcf75ae8010952bd96c619ad56c9458c3fb0e153a`）、`render.mjs`、[25 张场景图与清单](ux-v1.0/assets/SHA256SUMS)、[浏览器验证记录](ux-v1.0/assets/verification.json)。当前记录来自隔离 HeadlessChrome 153：25 张图、15 条检查、0 个页面错误；清单共 26 条（原型源加 25 张图）。检查记录包含 1440×900 母版、1920×1080 的 1.2×/96px 留白、2560×1440 的 1.6×/128px 留白、1400×860/1280×800 的 1×下限，以及滚轮/End纵向可达、归一化几何、真实点击、冲突在版本/能力/刷新往返中的保持、版本往返与焦点检查。
...
| V1.1-review | 2026-09-18建立整体审阅稿；2026-09-19核对用户原话，纠正旧双尺寸规则为同一母版整体缩放。修正图稿、文本与验证记录已按同一版本整合；待operator审查，不是已采纳版本 |
