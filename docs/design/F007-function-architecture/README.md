...
> **讨论稿 v0.2，功能与整体方案未定稿。** 本文将前期讨论、五视角图稿和当前设计依据汇集为可逐项评审的整体方案。2026-09-20 co-creator 授权补齐本文，不代表批准全部功能、技术选型或启动实现。五视角图稿版本为 **v4 候选稿**，两张展开流程图为 **v1**，均与本文版本分别管理；v2、v3 原图及原始流程定义保留。
...
- **实现观察**：在本次工作副本读取到相应代码；不证明已合入 main、部署或通过产品验收。
...
本次实现观察的读取点为 `010de0eeb5956be8a544d297e5d4bab0151a2f34`，当时工作副本处于 detached HEAD；它不是本次发布或验收基线。Kimi v4 图内“实现”沿用 `64100c2` 标识，**不能直接当作本次代码状态清单**。五张图均为候选概览，各节紧随的说明补足层次与限制；不声明图稿已全部复审通过。旧[产品架构文档](../../architecture/traqen-product-architecture.zh-CN.md)保留作历史参照，本轮不修改其发布状态或其他 Feature 真相源。
...
![Traqen 业务架构 v4 候选概览](images/kimi-f007v4-biz.png)

图分为**参与角色、业务能力、价值产出、贯穿治理**四部分。B01–B07 间的箭头表示成果如何互相支撑，不是七个必须串行执行的页面，也不是七个微服务。下表展开每个能力的工作与交接。
...
| **B03 端到端关联与质量追溯**／R2、R3 | 功能与规则组织；关联设计／代码／API／配置／测试／部署；双向查询；断链定位 | 业务主张、事实、材料引用、执行记录 → 同一模型的业务、实现、覆盖和追溯视图 | **V2 质量可解释**：每条结论能回到依据。F003 已规定三种视图；无依据的关联不能补成成立关系 |
...
| **B05 变更影响与持续回归**／R2、R3 | 比较前后版本；解释差异；遍历影响依据；识别陈旧结论；给出重验证建议 | 两个版本及各自证据 → CONFIRMED／POSSIBLE／UNKNOWN 的影响与重验证项 | **V2＋V3**：确定要复查什么。F004 首版只给建议，不自动阻止工作、合并或部署 |
...
| **B07 质量态势与交付证明**／R4，R1、R3 参与 | 按功能／版本／部署汇总；展开证据链；显示剩余风险；组织交付材料 | 图谱、执行证据、缺口及决定 → 有统计口径的质量视图与交付依据 | **V1＋V3 交付可查证**：验收能追到记录。报表模板、导出包、签署与交付流程为候选范围，不能由指标代码推出已交付 |
...
| B18 人工决定与审批 | 将有权人的决定绑定到具体命题、版本及范围 | 图中单人／双人／合规／破窗来自已有治理实现；哪些业务采用何种模式仍待确认 |
...
![Traqen 系统上下文 v4 候选图](images/kimi-f007v4-system.png)

本图回答“谁与平台交互”，内部仍是黑盒。E3–E5 是平台运行依赖，不与 Git、被测环境一概视为外部业务系统；“单节点”也不是逻辑系统边界。下表补足图中存储总线只画写入方向的简化。

| 节点 | 边界归属与职责 | 输入／输出与限制 |
|---|---|---|
| R1–R4、Web、CLI | 人的交互入口；操作 CLI 与分析模型 CLI 是不同用途 | 提交操作／查询，返回版本、结果或可处理错误；不能绕过授权 |
...
| E2 上传目录 | 受托材料入口 | 经 F001 采集核验进入来源包；不能越过冻结和准入读取任意目录 |
| E3 PostgreSQL | 平台结构化持久存储依赖 | 事务写入与查询读取；不是来源字节的替代物，F002 目标物理模型未因此定稿 |
| E4 持久文件卷 | F001 来源字节及受保护检查点的存储依赖 | 采集写入、受控读取和备份读取；不能依赖原仓库一直在线 |
| E5 独立故障域备份 | 平台恢复依赖 | 保存同一恢复水位的数据库与引用字节；恢复先在隔离环境核验 |
| E6 外部 Runner／CI | 被测环境侧执行边界 | 接收受控任务，返回签名执行证据；已有签名约束不等于传输已接线，任意 CI 也不自动可信 |

模型 CLI、模型提供方与 Agent 调查单元未在这张上下文概览充分展开，见下一层。系统图不能据此省略材料外发、凭据和进程边界。
...
为避免线条交叉，图分为操作入口、来源与调查、模型执行、测试执行、持久化与查询五区；**同号节点是同一职责，重复展示不表示新增运行实例**。编号仅在本展开图内使用，不与 v4 概览或应用图的同名编号自动对应。
...
![Traqen 应用架构 v4 候选图](images/kimi-f007v4-app.png)

**候选组织是模块化应用，而非按 B01–B07 各建一个服务。** 接入层处理协议和身份；应用层编排用例、事务及长任务；领域模块判断规则与状态；基础设施提供存储、材料读取、CLI 和外部执行适配。领域计算不应依赖 Web 页面或数据库驱动，应用编排依赖存储端口，再由启动入口装配适配器。现有 `source-truth/` 同时包含规则和 I/O，不能因图上位置就将整个目录认定为纯领域模型。

| 图中节点 | 软件职责与输入产出 | 依赖／实现观察与限制 |
|---|---|---|
| A1 Web UI | 当前 Workspace、版本选择、图谱、审核与证据查看 | `web/`；Next／React 与图形组件属于展示实现。图中 Cloudflare 发布表述只证明存在构建目标，不证明当前部署已接线 |
...
**v4 图的解释限制：** `S2 → X1` 任务下发尚无本轮核实的用例接线依据，本文将测试执行调度单列待定，不能按此箭头直接实施；“零 Web 框架”仅描述 API，不描述 React／Next 前端；“同进程调用”不涵盖模型 CLI 和目标 Runner。下列流程给出职责交接，涉及目标接口的步骤仍是设计要求。
...
![Traqen 数据架构 v4 候选图](images/kimi-f007v4-data.png)

图中 G1–G4 分别组织**身份与治理、来源与事实、验证、变更与缺口**。它是概念关系概览，混合了现有对象与设计对象，不能直接转换为数据库 DDL。尤其自动收录入图的字段与 Claim 的关系仍待 F003 定义；橙线不表示 Agent 已能创建具有人工权威的 Claim。
...
| G4 TraceGap／N1 处置生命周期 | 追溯链中的缺失、陈旧、冲突、失败及后续处置 | 可直接来自链评估，不必须先有 ChangeSet；图中登记→分派→处置→关闭是候选业务流程，权限与关闭条件未定 |
...
v4 中 `SnapshotManifest → FactBundle`、`Evidence → VerificationResult` 的 `1..*` **不能作为全生命周期合同照抄**：未提取快照可以暂无事实，未评估证据可以暂无验证结论。`ArtifactInventory → SnapshotManifest` 也需先厘清来源清单与复合部署快照的角色。本文不凭成功样例冻结这些关系；目标 ER、DDL 与 API Schema 列入 O04。
...
- **领域历史与可变头分开。** 已发布内容、决定、运行结果和证据保留不可变身份；当前图谱头、配置草稿、用户视图偏好有受控更新。图中“全部 append-only／versioned”不表示所有数据库行永远不可更新。
...
![Traqen 部署架构 v4 候选图](images/kimi-f007v4-deploy.png)
...
| T1 网络方案 | 浏览器入口与源站回源的加密和认证路径 | 见下节；不能把 Node 裸 HTTP 推论为当前 Cloudflare 必然在 HTTP 回源 |
| N1 首期承诺边界 | 说明单节点的能力限制 | 尚无本版批准的吞吐、并发、可用性和 RPO／RTO 数值 |

**代码配置核对：** [production-server.js](../../../src/api/production-server.js) 默认监听 `0.0.0.0:3000`，Node API 要求见根 [package.json](../../../package.json)；前端构建另见 [web/package.json](../../../web/package.json)（Node ≥22.13）。PostgreSQL TLS 默认 `require` 并校验证书，但代码也接受 `disable`／`no-verify`，所以图中“SSL 必需”应视为待落实的部署要求，不能宣称代码已全面禁止非校验连接。`SOURCE_TRUTH_CONFIG` 缺省可不装配来源运行时；“配置错误会拒绝启动”不等于“所有 API 启动都必然已具备 F001 来源能力”。
...
**当前五张 v4 为 Kimi 绘制并经多轮修订的候选图。** 本次直接关联已归入 Traqen 的原图，没有重绘、覆盖或将其升级为通过验收。文档 v0.1 补入分层说明、流程与待定项；v0.2 将两张展开图预渲染为 PNG，统一蓝色分区、编号节点和橙色分支，避免阅读器缺少 Mermaid 渲染能力时只显示空框。此次调整不改变功能边界或定稿状态。

| 视角 | 当前 v4 候选图 | v3 对照 | v2 对照 | 绘图源 |
|---|---|---|---|---|
| 业务 | [v4](images/kimi-f007v4-biz.png) | [v3](images/kimi-f007v3-biz.png) | [v2](images/kimi-f007v2-biz.png) | [biz.html](src/biz.html) |
| 系统 | [v4](images/kimi-f007v4-system.png) | [v3](images/kimi-f007v3-system.png) | [v2](images/kimi-f007v2-system.png) | [system.html](src/system.html) |
| 应用 | [v4](images/kimi-f007v4-app.png) | [v3](images/kimi-f007v3-app.png) | [v2](images/kimi-f007v2-app.png) | [app.html](src/app.html) |
| 数据 | [v4](images/kimi-f007v4-data.png) | [v3](images/kimi-f007v3-data.png) | [v2](images/kimi-f007v2-data.png) | [data.html](src/data.html) |
| 部署 | [v4](images/kimi-f007v4-deploy.png) | [v3](images/kimi-f007v3-deploy.png) | [v2](images/kimi-f007v2-deploy.png) | [deploy.html](src/deploy.html) |
...
五张 v4 图的共享样式为 [style.css](src/style.css)，是一套工作图源，不声称含各历史版本的独立源码快照；历史 PNG 保留供对照。v3 曾同名覆盖修订，不能仅凭“v3”标签识别其中某次中间稿。

图源预览：在本目录的 `src/` 内运行 `python3 -m http.server 8471 --bind 127.0.0.1`，逐页访问 `http://127.0.0.1:8471/biz.html` 等五页，再按固定浏览器、字体与视口全页截图。服务只绑定本机；当前 PNG 不承诺在任意环境逐字节重现。不要覆盖历史图；新版本使用新文件名，登记来源和校验值，并同时核对节点、连线与正文语义。
