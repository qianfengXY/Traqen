...
| 验收 | 后端 2 项 RED→GREEN；Web 2 项 RED→GREEN；真实 PG 包含 HTTP 的 3 项定向测试通过、零跳过；整仓严格类型与零警告 lint 通过。完整新提交回归及真实浏览器交互仍需验证。 |
...
本次尚不声明完整 quality gate：新提交整组、浏览器矩阵、当前 Git 拓扑 100k、两位独立 exact-HEAD 审阅、合入与隔离运行验收仍未闭合。旧 100k 通过和两次超时均保留。主工作区设计文档及旧素材未改。
