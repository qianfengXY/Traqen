...
- **Design/implementation** shows the versioned design document, processing flow, key decisions, and locatable concrete code blocks. Each code block binds its file, starting line, language, and Fact ID so an unlocatable summary cannot masquerade as implementation evidence.
