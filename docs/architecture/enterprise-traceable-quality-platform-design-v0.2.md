...
> 系统需求真相源：[Traqen 系统需求](traqen-system-requirements.zh-CN.md)。本文继续作为详细子系统架构。
...
功能说明与测试策略各自使用一个连续文档容器；章节之间通过标题、留白和分隔线形成层级，不得把每个字段包装成独立卡片，以免在宽屏上造成碎片化阅读。
...
- **设计实现**必须绑定仓库中的版本化 Markdown 设计文件，默认以文档阅读模式展示文件内容，同时允许查看原始 Markdown。代码视图在绑定 Fact 的业务逻辑代码块和完整原始源文件之间切换；每个代码块绑定文件、起始行、语言和 Fact ID，避免把无法定位的概述当成实现证据。
