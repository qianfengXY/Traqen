...
- The browser writes an active-run checkpoint after bounded groups of file and model work units. `RUNNING` and `PAUSED` are durable, distinct states: a refresh reattaches a `RUNNING` task automatically, while only the explicit Pause action writes `PAUSED`, which remains paused until the user resumes it. Recovery keeps the same run identity and reuses compatible file records and model classifications, so completed work units are neither replanned nor analyzed again. Compact completed-result summaries are retained separately for a later history-query surface.
...
- Project summaries, derived analysis records, active-run checkpoints, and supported directory handles survive a full refresh on the same browser profile. A running task uses the retained handle to continue automatically. If the browser revokes directory permission, the task keeps its durable run state and asks the user to authorize the original root before work can continue; it is not silently converted into a pause.
