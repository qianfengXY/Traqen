...
F002 将合格的 F001 `SourceTruthReceipt` 及其 `SourceSnapshot` 转换成带版本的 `Fact` 和 `EvidenceLink` 记录。它发布 API 结构树：当语言适配器能够证明时，路由、方法、声明式契约与源码处理器都属于直接观察。若提取器无法建立关系，则发布缺口。
...
- 面向已支持语言和框架的确定性提取器；只有获得 F001 `READY` 或 `READY_WITH_ACCEPTED_GAPS` Receipt 及其继承 Gap 后才可准入。
...
1. 架构师选择一个 F001 `READY` 或 `READY_WITH_ACCEPTED_GAPS` 的 Source Truth Receipt。
...
- [ ] F002 拒绝直接来源路径、branch 名、脏工作区、未完成快照或 `BLOCKED` Receipt 作为输入。
...
F002 依赖 F001 的合格不可变 Receipt、快照、完整清单及其继承 Gap。F003 可把事实作为证据，但必须保持确定性事实与语义候选之间的边界。F004 使用事实与缺口计算有边界的建议性影响。
