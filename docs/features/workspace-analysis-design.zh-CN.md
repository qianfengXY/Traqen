...
- 浏览器会在若干有界文件与模型工作单元后写入活动运行检查点。`RUNNING` 与 `PAUSED` 是持久且互不混淆的状态：刷新后会自动接续 `RUNNING` 任务；只有用户明确点击“暂停”才写入 `PAUSED`，并且必须由用户点击“继续分析”才能恢复。恢复过程沿用同一个运行身份，复用兼容的文件记录与模型分类，已经完成的工作单元既不会重新规划，也不会重复分析。紧凑的完成结果摘要独立保留，供后续历史查询栏目使用。
...
- 项目摘要、派生分析记录、活动运行检查点和受支持的目录句柄在同一浏览器配置中完整刷新后仍然存在。运行中的任务会使用已保存句柄自动接续；如果浏览器撤销了目录权限，任务保留原运行状态并要求用户重新授权原工程根目录，不会被静默改写成暂停。
