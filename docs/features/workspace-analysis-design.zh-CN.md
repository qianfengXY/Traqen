...
- 浏览器可以为有界的确定性源码准备写入检查点，但绝不拥有或宣称一个正在运行的 AnalysisRun。派生观察被接受后，API 创建唯一且持久的服务端 AnalysisRun。页面挂载、刷新、重连和轮询只发送读取请求；只有用户明确点击“暂停”或“恢复任务”才改变生命周期状态。恢复沿用同一个 run 身份和服务端检查点，已经完成的 WorkUnit 既不会重新规划，也不会重复分析。
...
- 项目摘要、派生分析记录、源码准备检查点、服务端 run 订阅和受支持的目录句柄在同一浏览器配置中完整刷新后仍然存在。运行中的服务端任务不依赖已保存的目录句柄，页面消失时仍会继续。目录权限被撤销只影响下一次本地重新扫描，不能暂停或恢复已经被服务端接受的 run。
