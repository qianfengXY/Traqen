...
- Workspace 创建入口是标题旁紧凑的 `+` 按钮，创建过程使用仅属于 Workspace 路由的对话框。切换到任何其他产品页面都会关闭该对话框，创建状态不会泄漏到其他页面。
...
- 分析 Agent 与产品其余部分统一使用全局主题变量和有界桌面布局。主 Agent 与子 Agent 对话在固定高度的区域内滚动，不再形成无限延伸的黑色终端页面。
