...
分析界面是真实的有界 Agent 会话，而不是给顺序日志换名称。顶部是流式主 Agent 对话，下面是三个固定高度的子 Agent 窗口；页面导航不会丢失活动任务、对话、耗时和进度。刷新、暂停或进程失败后，系统从 IndexedDB 最近检查点恢复可续跑会话。确定性提取完成后，“继续分析”直接恢复已保存 Snapshot 与未完成模型 WorkUnit，不重新遍历源码目录。只有证据策略与模型 Profile 一致时才复用记录和分类。每个检查点都会生成阶段性候选树投影；IndexedDB 只是缓存与检查点，不是真相源。项目快照与活动检查点把当前投影和大体量扫描记录分开保存。公开消息展示目标、有界输入、证据、不确定性、检查点与下一步，但不展示原始模型 JSON 或私有思维链。三个浏览器队列从同一 Source Manifest 分配有界语义任务；可选外部分析源在没有真实配置时必须标为不可用。
