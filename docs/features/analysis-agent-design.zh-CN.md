...
分析 Agent 不批准 Claim、不执行测试，也不会把 LLM 说法转成业务事实。带认证的 Traqen API 持有 Hybrid 执行、模型调用、凭据、检查点和生命周期变更；浏览器只准备派生观察并订阅进度。模型生成的名称、业务分组、置信度和依据在受治理的人工确认前都只是候选元数据。多实例 Worker 租约和分布式队列属于部署基础设施。OpenAPI YAML 与更多语言的确定性 AST 适配器仍是明确的后续扩展，不会伪装成“已完整分析”。
