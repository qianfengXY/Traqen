...
3. 服务端每完成一个 Analysis WorkUnit 就持久化检查点。目标生命周期也会持久化每个已完成扫描 WorkUnit；浏览器本地批次记录只能作为兼容缓存，不能承担正在执行的任务契约。
4. 第一次分析为全量。后续 `AUTO` 分析以最新完成结果为增量基线；调用方也可以显式指定 `FULL` 或 `INCREMENTAL`。
5. 分析只能通过精确候选键、稳定证据重合度或语义名称相似度提出 Candidate lineage 建议，不能分配、匹配或继承受治理的 Feature 身份。
6. Candidate lineage 与变化分类只是 `identityDecision: NOT_MADE` 的建议元数据。只有明确的治理 Decision 才能创建或关联不透明 `Feature.id` 并形成业务权威。
7. 当前候选树只包含当前观察。缺失观察进入 `candidateAbsences[].NO_CURRENT_OBSERVATION`，不是 Feature retirement 事件。
...
`Source Snapshot → [确定性 Fact 提取 || 直接源码 Skills] → WorkUnit-bound CandidateBundle → 确定性证据校验 → Candidate lineage 建议 → 只读候选投影`
...
结果会保存节点和关系的语义指纹。增量运行只规划有界邻域与变化 Fact 相交的 WorkUnit；未变化候选观察会在新运行中形成新的不可变 Candidate。候选对账记录以下变化类型：
...
每次运行都创建新的 Candidate ID，并只保留非权威的 `previousCandidateId` 建议。所有匹配都只是 `SUGGESTED`，`governedFeatureId` 始终为 null，也不会从治理 Feature 存储复制任何权威。当前运行未观察到的候选写入 `candidateAbsences.NO_CURRENT_OBSERVATION`；这绝不表示 Feature 已退役。
...
浏览器必须先取得一个已验证模型配置，才能开始新的 Workspace 分析。它只在本地完成有界的确定性源码准备，并发送不含代码的派生观察：候选名称、路径、说明、脱敏配置线索、测试文件线索、诊断和完整性。API 校验这些观察，将其规范化为仅含源码组件的 Snapshot 与 canonical Facts，再提交唯一的异步 AnalysisRun。模型规划、有界 WorkUnit 执行、证据校验、重试、检查点、暂停、恢复和最终候选投影全部由服务端持有。项目原始文件既不上传也不持久化；IndexedDB 只保存派生扫描记录、准备检查点和权威服务端 run 的订阅。
...
分析界面是有界 Agent 会话的投影，而不是给顺序日志换名称。顶部是主 Agent 对话，下面是三个固定高度的子 Agent 窗口，四个窗口都在内部滚动。API 接受 run 后，页面只充当订阅者：挂载、刷新、重连和轮询只读取同一个服务端检查点，不发送 Start、Pause 或 Resume。用户明确点击“暂停”和“恢复任务”时才会操作同一个 run ID；服务端不会重新规划或重复执行已完成 WorkUnit。IndexedDB 只是本地准备和 run 订阅的缓存，服务端检查点与结果才是真相源。公开消息展示目标、有界输入、证据、不确定性、检查点与下一步，但不展示原始模型 JSON 或私有思维链。

页面展示的是主 Agent 对账后的候选树，不是扫描符号浏览器，也不是治理 Feature 树。每个子 Agent WorkUnit 返回后，确定性代码先校验 CandidateBundle，再把有界结论分类到业务/API 候选投影并保存检查点。通过证据校验绝不等于业务确认。代表用户可识别行为的端点 Candidate 可以同时出现于两类投影，但节点类型始终是 `CANDIDATE_FEATURE`，`governedFeatureId` 始终为 null。只有确定性端点而没有子 Agent 结果时，只能展示低置信度临时 API Candidate。候选 taxonomy 使用**业务大模块 → 业务小模块 → 用户可识别行为**标签；源码目录、包名、类名、框架名和原始符号不得成为层级名称。多个观察可按已校验业务匹配键聚合并保留所有证据，但该键绝不是 Feature ID。

每个服务端子 Agent 槽位都有代次和上下文字符安全预算。达到预算的 70% 后，Worker 不再领取新工作，但会先完成当前原子工作单元、保存紧凑交接摘要和检查点、结束当前代次，再在同一可视槽位启动下一代。已完成 WorkUnit 保留候选输出，不会重复调用。未来可以把 Claude Code 或 Codex CLI 接在同一个服务端运行时/事件边界后面，但实现这种交互不依赖它们。
...
可以保留多个已扫描项目，同时只在侧栏展示选中的项目。从展示中移出是非破坏操作：Workspace 管理仍会读取其轻量摘要，但不会加载源码索引、候选树和追溯快照；重新勾选即可按需打开，无需重扫。

候选树提供两种投影。业务模式会过滤接口、命令、Repository、Adapter、Interface、Utils、配置代码和其他技术支持符号；API 模式展示接口线索以及匹配到的入口/调用实现代码块。两种投影都来自同一个 Workspace 最新分析结果，且都不携带受治理 Feature 权威。

## 明确边界

分析 Agent 不批准 Claim、不执行测试，也不会把 LLM 说法转成业务事实。带认证的 Traqen API 持有 Hybrid 执行、模型调用、凭据、检查点和生命周期变更。当前版本仍由浏览器在服务端 run 之前准备派生观察；目标生命周期中，浏览器只发送命令和订阅进度，Snapshot 捕获与扫描由服务端 Local Runner 持有。模型生成的名称、业务分组、置信度和依据在受治理的人工确认前都只是候选元数据。多实例 Worker 租约和分布式队列属于部署基础设施。OpenAPI YAML 与更多语言的确定性 AST 适配器仍是明确的后续扩展，不会伪装成“已完整分析”。
