...
分析任务执行台在执行前展示计划，执行中持续报告当前 Profile、传输策略、当前阶段、文件与模型批次计数、加权总进度、累计耗时、检查点、结构校验摘要以及完成、暂停或失败事件。针对每一次 LLM 调用，它会实时展示请求 ID、接口地址、提示词/证据预览、输入规模、网关状态与首字节耗时、响应数据块和字符进度、结构化输出预览、供应商返回的 Token 用量（若有）、校验结果和调用耗时；提示词与结构化结果可展开查看。API Key、Authorization Header 和模型私有思维链绝不展示。
