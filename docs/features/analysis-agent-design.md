...
The browser requires a verified model profile before starting a new Workspace analysis. It performs bounded deterministic source preparation locally and sends only code-free derived observations: candidate names, paths, descriptions, redacted configuration clues, test-asset clues, diagnostics, and completeness. The API validates and normalizes those observations into a source-only Snapshot and canonical Facts, then submits one asynchronous AnalysisRun. Model planning, bounded WorkUnit execution, evidence validation, retry, checkpointing, pause, resume, and final Candidate projection are server-owned. Raw project files are neither uploaded nor persisted; IndexedDB contains derived scan records, a preparation checkpoint, and a subscription to the authoritative server run.
...
The analysis surface is a bounded Agent-session projection rather than a relabeled sequential log. A Main Agent conversation sits above exactly three child-Agent windows, and all four windows have fixed visible heights with internal scrolling. Once the API accepts the run, the page is only a subscriber: mount, reload, reconnect, and polling read the same server checkpoint without issuing Start, Pause, or Resume. Explicit Pause and Resume commands target the same run ID, and the server never replans or re-executes completed WorkUnits. IndexedDB is only a cache for local preparation and the run subscription; the server checkpoint and result are truth. Public messages expose goals, bounded inputs, evidence, uncertainty, checkpoint state, and next actions without rendering raw model JSON or private chain-of-thought.
...
Every server child slot has a generation and a context-character safety budget. At 70% of the configured budget, the worker stops claiming new work after its current atomic unit, saves a compact handoff/checkpoint, retires that generation, and starts the next generation in the same visual slot. Completed WorkUnits retain their Candidate output and are not repeated. Claude Code or Codex CLI can later be added behind the same server runtime/event boundary but are not required to obtain the interaction model.
