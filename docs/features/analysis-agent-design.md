...
The analysis surface is an Agent session rather than a raw request debugger. Before execution it establishes one explicit main task and a visible subtask tree: build the source manifest, extract locatable evidence, resolve business and API semantics through bounded module-oriented child tasks, then validate and update the Workspace. During execution its activity stream reports the responsible Agent role, current action, evidence-backed stage discovery, validation result, checkpoint, elapsed time, pause, failure, and final outcome in concise human-readable language. It reports observable orchestration and results, not fabricated or private model reasoning.
