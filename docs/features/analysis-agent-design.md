...
The analysis surface is a real bounded Agent session rather than a relabeled sequential log. A streaming Main Agent conversation sits above exactly three child-Agent windows. All four windows have a fixed visible height and scroll internally, so long runs never grow the page without bound. The Workspace analysis surface remains mounted while the user visits traceability, graph, review, impact, or metrics pages, preserving the active task, transcripts, elapsed time, and progress. After an actual page reload, pause, or process failure, the latest persisted IndexedDB checkpoint is restored as a resumable session with its completed/total progress. Once deterministic extraction has completed and the run has entered model enrichment, “Continue analysis” restores the complete saved scan snapshot and unfinished model work units directly; it must not traverse the source directory or repeat scanning. Only a checkpoint that is still in deterministic extraction needs source-directory access for its unfinished files. On browsers that support the File System Access API, Traqen persists that directory handle per Workspace and requests read permission only when this remaining file work requires it. Browsers without this API use the directory-input fallback and cannot persist a reusable handle; an existing project created before this capability needs one migration-time directory authorization. Matching file records are always reused; completed model classifications are reused when the same profile continues, while changing profiles reuses extraction but reclassifies candidates under the new profile. Every persisted file or model checkpoint also produces a progressive Feature-tree projection. Completed discoveries and hierarchical statistics therefore appear during analysis and remain visible after pause or reload instead of waiting for the entire run. Project snapshots and active checkpoints persist their current analysis projection separately from bulk scanner records. Refresh loads the projection and task summary first; the full file-record set is read lazily only when incremental or resumed analysis actually starts. Public messages follow a compact Codex-style working contract: the Main Agent exposes goal, plan, evidence basis, risks, assignments, reconciliation findings, and next action; child Agents expose a real current-task name, goal, bounded input, action, findings, evidence, uncertainty, checkpoint, and next action. Raw structured model JSON is not rendered as ordinary conversation; after validation, the UI projects it into an auditable public reasoning summary containing observations, judgment method, counter-checks, uncertainty, stage conclusions, and the next action. This is evidence-backed process transparency, not private chain-of-thought. The current browser orchestration uses three bounded semantic-analysis queues partitioned from the same independent Source Manifest; deterministic extraction runs before those queues. ECC-class repository understanding and Specone-class specification reversal remain optional external analysis sources and must be labelled unavailable until genuinely configured. The Main Agent controls queue assignment, pause, generation handoff, and final reconciliation without substituting scanner candidates for a missing Skill.

The visible Feature tree is a Main-Agent reconciliation projection, not a scanner-symbol browser. After every child work unit returns, the Main Agent matches each child conclusion to the bounded scanner candidates, records one of four explicit outcomes—business admission, API admission, technical exclusion, or pending Agent conclusion—then checkpoints and refreshes both trees immediately. An Agent-confirmed endpoint may enter both trees when it represents a user-recognizable business function; it is no longer discarded merely because its scanner kind is `ENDPOINT`. If a child response is unavailable but deterministic extraction proves an endpoint definition, the Main Agent may expose it only as a low-confidence provisional API entry. It cannot enter the business tree, and it remains eligible for later model classification. The business tree uses **business module → business submodule → user-recognizable function** levels supplied by the Agent; source folders, packages, classes, frameworks, and raw code-symbol names cannot become hierarchy labels. Multiple scanner candidates with the same validated stable business key are merged into one function while retaining all source, configuration, test, and implementation evidence references. Classifications created under an older hierarchy policy stay hidden and become pending model work instead of leaking legacy scanner labels into the tree. The Workspace dashboard initially expands every available hierarchy level and provides explicit expand-all and collapse controls.
