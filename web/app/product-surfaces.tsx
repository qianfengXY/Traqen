  ...
  return <>
    ...
    {!artifact ? <Unavailable t={t} reason={t("尚无已发布图谱。完成一次 Workspace 分析后再浏览功能与 API。", "No published graph exists. Complete Workspace analysis before browsing Features and APIs.")} /> : <section className="explorer-layout panel">
      ...
      <div className="explorer-detail">{selected && <><header><div><p className="eyebrow">{selected.type} · {historical ? "HISTORICAL" : "CURRENT"}</p><h2>{selected.label ?? selected.id}</h2><span className={`authority-pill ${authorityLabel(selected.authority).toLowerCase()}`}>{authorityLabel(selected.authority)}</span></div><button className="button" disabled={!governedSelection} onClick={onOpenGraph}>{t("在图谱中聚焦", "Focus in graph")} →</button></header><div className="detail-tabs" role="tablist" aria-label={t("详情视图", "Detail views")}>{featureDetailTabs.map((tab) => <button key={tab} role="tab" aria-selected={activeTab === tab} className={activeTab === tab ? "active" : ""} onClick={() => setActiveTab(tab)}>{tabLabels[tab]}{tab === "gaps" && detail ? <em>{detail.gaps.items.length}</em> : null}</button>)}</div>
        ...
        {!governedSelection ? <div className="candidate-boundary"><b>{selected.authority}</b><p>{t("该对象不属于 Published governed tree。候选与 Gap 不能借用当前已治理详情伪装成权威证据。", "This object is outside the Published governed tree. Candidate and Gap objects cannot borrow governed detail as authoritative evidence.")}</p></div> : historicalAvailability ? <div className="historical-compatibility-state" role="alert"><b>UNAVAILABLE · REANALYSIS REQUIRED</b><h3>{t("该旧版本没有不可变追溯快照", "This legacy revision has no immutable traceability snapshot")}</h3><p>{t("系统保留了所选 Revision、Feature/API、Snapshot 与 artifact digest，且没有用当前数据伪造历史详情。请返回当前版本，或在 Workspace 分析中基于受治理输入重新分析。", "The selected Revision, Feature/API, Snapshot, and artifact digest remain visible, and current data was not substituted for missing history. Return to the current revision or run governed reanalysis from Workspace analysis.")}</p><dl><dt>Reason</dt><dd>{historicalAvailability.reasonCode}</dd><dt>Revision</dt><dd>{historicalAvailability.graphRevisionId}</dd><dt>Snapshot</dt><dd>{historicalAvailability.snapshotManifestId}</dd><dt>Artifact digest</dt><dd>{historicalAvailability.graphArtifactDigest}</dd><dt>Recovery</dt><dd>{historicalAvailability.recovery.action}</dd></dl><button className="button primary" onClick={() => onSelectRevision("current")}>{t("查看当前 Published Head", "View current Published Head")}</button></div> : !detail ? <Unavailable t={t} reason={t("所选 Snapshot 的追溯响应不可用；所有证据保持 MISSING。", "Traceability is unavailable for the selected Snapshot; all evidence remains MISSING.")} /> : <div className="detail-view" role="tabpanel">
...
export function GraphExplorer({ t, workspaceId, artifact, revision, revisions, historical, focusedId, graph, path, loading, error, onFocus, onSelectRevision, onLoadGraph, onQueryPath, onResolveEvidence }: {
  ...
  return <>
    ...
    {graph?.historicalAvailability && <div className="historical-banner" role="alert"><b>LEGACY ARTIFACT · TRACEABILITY SNAPSHOT UNAVAILABLE</b><span>{t("节点、边与路径来自所选旧 artifact；缺失的 F002 历史详情不会用当前数据补写。", "Nodes, edges, and paths come from the selected legacy artifact; missing F002 history is never backfilled with current data.")}</span></div>}
