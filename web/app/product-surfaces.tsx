...
import type { CapabilityKey, ChildCapabilityRole, EffectiveCapabilityCatalog, ExecutionProfile, GlobalModelProfile, ReviewQueueItem, WorkspaceCapabilityDraft } from "./product-foundation-client";
...
import type { CurrentUnderstandingGraph, FeatureGraphPathResult, FeatureGraphProjection, FeatureTraceability, FeatureUnderstandingHistory, GraphRevision, HistoricalAvailability, ResolvedGraphEvidence } from "./understanding-graph-client";
...
export function FeatureExplorer({ t, workspaceId, artifact, revision, revisions, historical, selectedId, history, traceability, graph, loading, error, working, onSelectRevision, onSelectNode, onOpenGraph, onReanalyzeHistorical }: {
  t: T;
  workspaceId: string;
  ...
  const detail = useMemo(() => traceability && !traceability.historicalAvailability && revision ? buildFeatureDetail(traceability, graph, {
    workspaceId,
  ...
  const historicalAvailability = traceability?.historicalAvailability ?? history?.historicalAvailability ?? null;
  useEffect(() => {
  ...
  return <>
    ...
    {!artifact ? <Unavailable t={t} reason={t("尚无已发布图谱。完成一次 Workspace 分析后再浏览功能与 API。", "No published graph exists. Complete Workspace analysis before browsing Features and APIs.")} /> : <section className="explorer-layout panel">
      ...
      <div className="explorer-detail">{selected && <><header><div><p className="eyebrow">{selected.type} · {historical ? "HISTORICAL" : "CURRENT"}</p><h2>{selected.label ?? selected.id}</h2><span className={`authority-pill ${authorityLabel(selected.authority).toLowerCase()}`}>{authorityLabel(selected.authority)}</span></div><button className="button" disabled={!governedSelection} onClick={onOpenGraph}>{t("在图谱中聚焦", "Focus in graph")} →</button></header><div className="detail-tabs" role="tablist" aria-label={t("详情视图", "Detail views")}>{featureDetailTabs.map((tab) => <button key={tab} role="tab" aria-selected={activeTab === tab} className={activeTab === tab ? "active" : ""} onClick={() => setActiveTab(tab)}>{tabLabels[tab]}{tab === "gaps" && detail ? <em>{detail.gaps.items.length}</em> : null}</button>)}</div>
        ...
        {!governedSelection ? <div className="candidate-boundary"><b>{selected.authority}</b><p>{t("该对象不属于 Published governed tree。候选与 Gap 不能借用当前已治理详情伪装成权威证据。", "This object is outside the Published governed tree. Candidate and Gap objects cannot borrow governed detail as authoritative evidence.")}</p></div> : historicalAvailability ? <div className="historical-compatibility-state" role="alert">
          <b>{historicalAvailability.recovery.executable ? "UNAVAILABLE · REANALYSIS REQUIRED" : "UNAVAILABLE · RECOVERY CONTEXT NOT RETAINED"}</b>
          <h3>{t("旧版不可变证据不足，无法安全展示追溯详情", "Legacy immutable evidence is insufficient for safe traceability detail")}</h3>
          <p>{historicalAvailability.recovery.executable
            ? t("系统保留了所选 Revision 的原始 Snapshot、SourceRegistration 与执行配置，可由服务端启动重分析并生成不移动当前 Head 的新历史 Revision。", "The server retained the selected Revision's original Snapshot, SourceRegistration, and execution profile, so it can create a new historical Revision without moving the current Head.")
            : t("系统仍保留所选历史身份，但没有核验到完整的原始恢复上下文，因此不会提供一个必然失败的重分析按钮，也不会借用当前 Workspace 状态。", "The selected historical identity remains visible, but its complete original recovery context could not be verified. No dead-end command or current Workspace substitute is offered.")}</p>
          <dl><dt>Reason</dt><dd>{historicalAvailability.reasonCode}</dd><dt>Revision</dt><dd>{historicalAvailability.graphRevisionId}</dd><dt>Snapshot</dt><dd>{historicalAvailability.snapshotManifestId}</dd><dt>Artifact digest</dt><dd>{historicalAvailability.graphArtifactDigest}</dd><dt>Recovery</dt><dd>{historicalAvailability.recovery.executable ? `${historicalAvailability.recovery.method} ${historicalAvailability.recovery.endpoint}` : `${historicalAvailability.recovery.reasonCode} · ${historicalAvailability.recovery.message}`}</dd></dl>
          <div className="heading-actions">{historicalAvailability.recovery.executable && <button className="button primary" disabled={working} onClick={() => onReanalyzeHistorical(historicalAvailability)}>{t("从不可变 Snapshot 重新分析", "Reanalyze immutable Snapshot")}</button>}<button className="button" disabled={working} onClick={() => onSelectRevision("current")}>{t("查看当前 Published Head", "View current Published Head")}</button></div>
        </div> : !detail ? <Unavailable t={t} reason={t("所选 Snapshot 的追溯响应不可用；所有证据保持 MISSING。", "Traceability is unavailable for the selected Snapshot; all evidence remains MISSING.")} /> : <div className="detail-view" role="tabpanel">
...
export function GraphExplorer({ t, workspaceId, artifact, revision, revisions, historical, focusedId, graph, path, loading, error, working, onFocus, onSelectRevision, onLoadGraph, onQueryPath, onResolveEvidence, onReanalyzeHistorical }: {
  t: T;
  workspaceId: string;
  artifact: GraphArtifact | null;
  revision: GraphRevision | null;
  ...
  return <>
    ...
    {graph?.historicalAvailability && <div className="historical-banner" role="alert"><b>LEGACY ARTIFACT · TRACEABILITY SNAPSHOT UNAVAILABLE</b><span>{graph.historicalAvailability.recovery.executable ? t("只显示可由旧 artifact 唯一证明归属的对象；服务端已核验原始恢复绑定。", "Only objects with provable legacy ownership are shown; the server verified the original recovery binding.") : t("只显示可由旧 artifact 唯一证明归属的对象；原始恢复上下文未保留，当前 Workspace 状态不会被替代使用。", "Only objects with provable legacy ownership are shown; the original recovery context was not retained and current Workspace state is never substituted.")}</span>{graph.historicalAvailability.recovery.executable ? <button className="button primary" disabled={working} onClick={() => onReanalyzeHistorical(graph.historicalAvailability!)}>{t("从不可变 Snapshot 重新分析", "Reanalyze immutable Snapshot")}</button> : <small>{graph.historicalAvailability.recovery.reasonCode}</small>}</div>}
...
export function GlobalModelLibrary({ t, models, working, onCreate, onVerify }: { t: T; models: GlobalModelProfile[]; working: boolean; onCreate: (input: Record<string, unknown>) => void; onVerify: (profileId: string) => void }) {
  ...
  const submit = () => {
    onCreate(transport === "API"
      ? { profileId, displayName, transport, endpoint, model, apiKey, providerAdapter: "OPENAI_COMPATIBLE" }
      : { profileId, displayName, transport, cliAdapter, ...(model ? { model } : {}), ...(executablePath ? { executablePath } : {}) });
    setApiKey("");
  };
  return <><section className="page-heading"><div><p className="eyebrow">F006 · Global settings</p><h1>{t("模型库", "Model library")}</h1><p>{t("模型是全局可复用连接资产；Workspace Agent 必须显式选择 READY revision。", "Models are reusable global connection assets. Workspace Agents must explicitly select a READY revision.")}</p></div><span className="authority-pill neutral">{models.length} PROFILES</span></section><section className="settings-layout"><div><article className="panel settings-card"><header className="panel-head"><div><h2>{t("已注册模型", "Registered models")}</h2><p>{t("验证仅证明传输和登录可用，不代表语义校准完成。", "Verification proves transport and login readiness, not semantic calibration.")}</p></div></header><div className="catalog-table model-catalog">{models.map((item) => <div key={item.profileId}><b>{item.displayName}</b><span>{item.transport}</span><span>{item.model ?? item.profileId}</span><span className={`status-chip ${item.readiness.toLowerCase()}`}>{item.readiness}</span><button className="button" disabled={working || item.lifecycle !== "ACTIVE"} onClick={() => onVerify(item.profileId)}>{t("验证", "Verify")}</button></div>)}{models.length === 0 && <p className="explicit-empty">{t("尚未配置模型。添加 API 连接或 allowlist 本地 CLI。", "No models configured. Add an API connection or an allowlisted local CLI.")}</p>}</div></article></div><aside><article className="panel config-summary"><p className="eyebrow">{t("添加 / 修订模型", "Add / revise model")}</p><label>{t("传输", "Transport")}<select value={transport} onChange={(event) => setTransport(event.currentTarget.value as "API" | "CLI")}><option>API</option><option>CLI</option></select></label><label>ID<input value={profileId} onChange={(event) => setProfileId(event.currentTarget.value)} /></label><label>{t("显示名", "Display name")}<input value={displayName} onChange={(event) => setDisplayName(event.currentTarget.value)} /></label>{transport === "API" ? <><label>Endpoint<input value={endpoint} onChange={(event) => setEndpoint(event.currentTarget.value)} placeholder="https://…/v1" /></label><label>Model<input value={model} onChange={(event) => setModel(event.currentTarget.value)} /></label><label>Token<input type="password" autoComplete="off" value={apiKey} onChange={(event) => setApiKey(event.currentTarget.value)} /></label></> : <><label>Adapter<select value={cliAdapter} onChange={(event) => setCliAdapter(event.currentTarget.value)}>{["CODEX", "CLAUDE", "GEMINI", "KIMI"].map((value) => <option key={value}>{value}</option>)}</select></label><label>{t("模型（可选）", "Model (optional)")}<input value={model} onChange={(event) => setModel(event.currentTarget.value)} /></label><label>{t("可执行文件路径（可选）", "Executable path (optional)")}<input value={executablePath} onChange={(event) => setExecutablePath(event.currentTarget.value)} /></label></>}<button className="button primary" disabled={working || !profileId.trim() || !displayName.trim() || (transport === "API" && (!endpoint.trim() || !model.trim() || !apiKey.trim()))} onClick={submit}>{t("保存模型", "Save model")}</button><small>{t("Token 只在密钥入口提交，普通响应、diff 与 revision 不返回明文。CLI adapter 直接构造 argv，永不启用 shell。", "Tokens enter only through the secret ingress and are never returned in ordinary responses, diffs, or revisions. CLI adapters construct argv directly and never enable a shell.")}</small></article></aside></section></>;
}

export function CapabilitySettings({ t, models, catalog, draft, profile, profileHistory, mainModel, setMainModel, mainSkillNames, setMainSkillNames, mainMcpNames, setMainMcpNames, childSlots, setChildSlots, disabledKeys, setDisabledKeys, working, onSave, onResolve }: {
  t: T; models: GlobalModelProfile[]; catalog: EffectiveCapabilityCatalog; draft: WorkspaceCapabilityDraft | null; profile: ExecutionProfile | null; profileHistory: ExecutionProfile[]; mainModel: string; setMainModel: (value: string) => void; mainSkillNames: string[]; setMainSkillNames: (value: string[]) => void; mainMcpNames: string[]; setMainMcpNames: (value: string[]) => void; childSlots: ChildCapabilityRole[]; setChildSlots: (value: ChildCapabilityRole[]) => void; disabledKeys: CapabilityKey[]; setDisabledKeys: (value: CapabilityKey[]) => void; working: boolean; onSave: () => void; onResolve: () => void;
}) {
  const readyModels = models.filter(({ readiness, lifecycle }) => readiness === "READY" && lifecycle === "ACTIVE");
  const skills = catalog.entries.filter(({ kind }) => kind === "SKILL");
  const mcps = catalog.entries.filter(({ kind }) => kind === "MCP");
  const enabled = (kind: "SKILL" | "MCP", name: string) => !disabledKeys.some((key) => key.kind === kind && key.normalizedName === name);
  const toggleDisabled = (kind: "SKILL" | "MCP", normalizedName: string) => setDisabledKeys(enabled(kind, normalizedName) ? [...disabledKeys, { kind, normalizedName }] : disabledKeys.filter((key) => key.kind !== kind || key.normalizedName !== normalizedName));
  const toggleGrant = (values: string[], setValues: (value: string[]) => void, name: string) => setValues(values.includes(name) ? values.filter((value) => value !== name) : [...values, name]);
  ...
  const capabilityPicker = (kind: "SKILL" | "MCP", entries: typeof skills, values: string[], setValues: (value: string[]) => void) => <div className="capability-picker" aria-label={`${kind} grants`}>{entries.filter(({ normalizedName }) => enabled(kind, normalizedName)).map((entry) => <label key={`${kind}-${entry.normalizedName}`}><input type="checkbox" checked={values.includes(entry.normalizedName)} onChange={() => toggleGrant(values, setValues, entry.normalizedName)} /><span>{entry.normalizedName}</span><small>{entry.source}{entry.projectRelation ? ` · ${entry.projectRelation}` : ""}</small></label>)}{entries.filter(({ normalizedName }) => enabled(kind, normalizedName)).length === 0 && <small>{t("当前有效目录为空", "The effective catalog is empty")}</small>}</div>;
  return <><section className="page-heading"><div><p className="eyebrow">F006 · Workspace policy</p><h1>{t("Workspace 能力设置", "Workspace capability settings")}</h1><p>{t("全局模型连接、项目能力目录和可编辑草稿彼此独立；激活后生成运行固定的不可变 Revision。", "Global model connections, the project capability catalog, and the editable draft are separate authorities. Activation creates an immutable run-pinned revision.")}</p></div>{profile ? <span className="authority-pill published">ACTIVE · v{profile.configVersion}</span> : <span className="authority-pill candidate">DRAFT</span>}</section><section className="capability-layers"><article className="active"><b>1</b><span>{t("模型库", "Model library")}<small>{readyModels.length}/{models.length} READY</small></span></article><article className="active"><b>2</b><span>{t("能力目录", "Capability catalog")}<small>{catalog.summary.effectiveCount} effective</small></span></article><article className="active"><b>3</b><span>Workspace Draft<small>{draft ? `v${draft.revision}` : t("未保存", "Unsaved")}</small></span></article><article className={profile ? "active" : ""}><b>4</b><span>{t("不可变 Revision", "Immutable revision")}<small>{shortId(profile?.id)}</small></span></article></section><section className="settings-layout"><div>
    <article className="panel settings-card"><header className="panel-head"><div><h2>{t("有效 Skill / MCP", "Effective Skills / MCPs")}</h2><p>{t("项目同 typed key 条目完整覆盖内置条目；禁用发生在覆盖之后。", "A project entry fully overlays a built-in entry with the same typed key; disable is applied after overlay.")}</p></div></header><div className="catalog-table">{[...skills, ...mcps].map((entry) => <div key={`${entry.kind}-${entry.normalizedName}`} className={!enabled(entry.kind, entry.normalizedName) ? "disabled" : ""}><b>{entry.normalizedName}</b><span>{entry.kind}</span><span>{entry.source}{entry.projectRelation ? ` · ${entry.projectRelation}` : ""}</span><label><input type="checkbox" checked={enabled(entry.kind, entry.normalizedName)} onChange={() => toggleDisabled(entry.kind, entry.normalizedName)} />{t("启用", "Enabled")}</label></div>)}{catalog.entries.length === 0 && <p className="explicit-empty">{t("内置与项目目录均为空；这是正常状态。", "Both built-in and project catalogs are empty; this is a normal state.")}</p>}</div></article>
    <article className="panel settings-card"><header className="panel-head"><div><h2>Main Agent</h2><p>{t("显式选择一个 READY 模型及允许调用的能力。", "Explicitly select one READY model and the capabilities this slot may invoke.")}</p></div></header><div className="form-grid"><label>{t("模型", "Model")}<select value={mainModel} onChange={(event) => setMainModel(event.currentTarget.value)}><option value="">{t("选择 READY 模型", "Select a READY model")}</option>{readyModels.map((model) => <option key={model.profileId} value={model.profileId}>{model.displayName} · {model.transport}</option>)}</select></label><div><b>Skills</b>{capabilityPicker("SKILL", skills, mainSkillNames, setMainSkillNames)}</div><div><b>MCP</b>{capabilityPicker("MCP", mcps, mainMcpNames, setMainMcpNames)}</div></div></article>
    <article className="panel settings-card"><header className="panel-head"><div><h2>Child Agent Slots</h2><p>{t("至少两个启用且完整的 Child；可以继续添加。", "At least two enabled and complete Child slots are required; more may be added.")}</p></div><button className="button" onClick={() => setChildSlots(addChildSlot(childSlots, { model: mainModel, skillNames: [], mcpNames: [] }))}>＋ {t("添加槽位", "Add slot")}</button></header><div className="child-slot-list">{childSlots.map((slot, index) => <div key={slot.id}><span>{slot.id}</span><label>{t("模型", "Model")}<select value={slot.model} onChange={(event) => updateSlot(index, { model: event.currentTarget.value })}><option value="">—</option>{readyModels.map((model) => <option key={model.profileId} value={model.profileId}>{model.displayName}</option>)}</select></label><div><b>Skills</b>{capabilityPicker("SKILL", skills, slot.skillNames, (skillNames) => updateSlot(index, { skillNames }))}</div><div><b>MCP</b>{capabilityPicker("MCP", mcps, slot.mcpNames, (mcpNames) => updateSlot(index, { mcpNames }))}</div><label>{t("独立组", "Independence group")}<input value={slot.independenceGroup} onChange={(event) => updateSlot(index, { independenceGroup: event.currentTarget.value })} /></label><button aria-label={t("删除槽位", "Remove slot")} disabled={childSlots.length <= 2} onClick={() => setChildSlots(removeChildSlot(childSlots, slot.id))}>×</button></div>)}</div></article>
  </div><aside><article className="panel config-summary"><p className="eyebrow">{t("解析摘要", "Resolution summary")}</p><h2>{profile ? t("已激活", "Activated") : draft ? t("草稿已保存", "Draft saved") : t("尚未保存", "Unsaved")}</h2><dl><dt>Built-in</dt><dd>{catalog.summary.builtinCount}</dd><dt>Overrides</dt><dd>{catalog.summary.projectOverrideCount}</dd><dt>Additions</dt><dd>{catalog.summary.projectAdditionCount}</dd><dt>Disabled</dt><dd>{disabledKeys.length}</dd><dt>Effective</dt><dd>{catalog.summary.effectiveCount - disabledKeys.filter((key) => !catalog.entries.some((entry) => entry.kind === key.kind && entry.normalizedName === key.normalizedName && entry.disabled)).length}</dd><dt>Child slots</dt><dd>{childSlots.length}</dd><dt>{t("历史 Revision", "Revision history")}</dt><dd>{profileHistory.length}</dd></dl><button className="button" disabled={working} onClick={onSave}>{t("保存草稿", "Save draft")}</button><button className="button primary" disabled={working || !draft || !mainModel || childSlots.length < 2 || childSlots.some(({ model, independenceGroup }) => !model || !independenceGroup)} onClick={onResolve}>{t("验证并激活", "Validate and activate")}</button><small>{t("无效草稿仍会保存。现有 Run 继续固定旧 Revision；只有后续 Run 使用新 Revision。", "Invalid drafts remain saved. Existing runs keep their pinned revision; only later runs use the new revision.")}</small></article></aside></section></>;
}
