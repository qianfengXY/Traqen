  ...
  return <>
    ...
    {!artifact ? <Unavailable t={t} reason={t("尚无已发布图谱。完成一次 Workspace 分析后再浏览功能与 API。", "No published graph exists. Complete Workspace analysis before browsing Features and APIs.")} /> : <section className="explorer-layout panel">
      ...
      <div className="explorer-detail">{selected && <><header><div><p className="eyebrow">{selected.type} · {historical ? "HISTORICAL" : "CURRENT"}</p><h2>{selected.label ?? selected.id}</h2><span className={`authority-pill ${authorityLabel(selected.authority).toLowerCase()}`}>{authorityLabel(selected.authority)}</span></div><button className="button" disabled={!governedSelection} onClick={onOpenGraph}>{t("在图谱中聚焦", "Focus in graph")} →</button></header><div className="detail-tabs" role="tablist" aria-label={t("详情视图", "Detail views")}>{featureDetailTabs.map((tab) => <button key={tab} role="tab" aria-selected={activeTab === tab} className={activeTab === tab ? "active" : ""} onClick={() => setActiveTab(tab)}>{tabLabels[tab]}{tab === "gaps" && detail ? <em>{detail.gaps.items.length}</em> : null}</button>)}</div>
        ...
        {!governedSelection ? <div className="candidate-boundary"><b>{selected.authority}</b><p>{t("该对象不属于 Published governed tree。候选与 Gap 不能借用当前已治理详情伪装成权威证据。", "This object is outside the Published governed tree. Candidate and Gap objects cannot borrow governed detail as authoritative evidence.")}</p></div> : historicalAvailability ? <div className="historical-compatibility-state" role="alert"><b>UNAVAILABLE · REANALYSIS REQUIRED</b><h3>{t("旧版不可变证据不足，无法安全展示追溯详情", "Legacy immutable evidence is insufficient for safe traceability detail")}</h3><p>{t("系统保留所选 Revision、Feature/API、Snapshot 与 artifact digest，且不会借用当前数据或另一 Feature 的证据。可从原不可变 Snapshot 启动服务端重分析；它会生成新的历史 Revision，且不会移动当前 Published Head。", "The selected Revision, Feature/API, Snapshot, and artifact digest remain visible. Current data and evidence owned by another Feature are never substituted. A server reanalysis can run from the original immutable Snapshot; it creates a new historical Revision without moving the current Published Head.")}</p><dl><dt>Reason</dt><dd>{historicalAvailability.reasonCode}</dd><dt>Revision</dt><dd>{historicalAvailability.graphRevisionId}</dd><dt>Snapshot</dt><dd>{historicalAvailability.snapshotManifestId}</dd><dt>Artifact digest</dt><dd>{historicalAvailability.graphArtifactDigest}</dd><dt>Recovery</dt><dd>{historicalAvailability.recovery.method} {historicalAvailability.recovery.endpoint}</dd></dl><div className="heading-actions"><button className="button primary" disabled={working} onClick={() => onReanalyzeHistorical(historicalAvailability)}>{t("从不可变 Snapshot 重新分析", "Reanalyze immutable Snapshot")}</button><button className="button" disabled={working} onClick={() => onSelectRevision("current")}>{t("查看当前 Published Head", "View current Published Head")}</button></div></div> : !detail ? <Unavailable t={t} reason={t("所选 Snapshot 的追溯响应不可用；所有证据保持 MISSING。", "Traceability is unavailable for the selected Snapshot; all evidence remains MISSING.")} /> : <div className="detail-view" role="tabpanel">
...
export function GraphExplorer({ t, workspaceId, artifact, revision, revisions, historical, focusedId, graph, path, loading, error, working, onFocus, onSelectRevision, onLoadGraph, onQueryPath, onResolveEvidence, onReanalyzeHistorical }: {
  ...
  return <>
    ...
    {graph?.historicalAvailability && <div className="historical-banner" role="alert"><b>LEGACY ARTIFACT · TRACEABILITY SNAPSHOT UNAVAILABLE</b><span>{t("只显示可由旧 artifact 唯一证明归属的节点、边与路径；缺失的 F002 历史详情不会用当前或其他 Feature 数据补写。", "Only nodes, edges, and paths with uniquely provable ownership in the legacy artifact are shown; missing F002 history is never backfilled with current or another Feature's data.")}</span><button className="button primary" disabled={working} onClick={() => onReanalyzeHistorical(graph.historicalAvailability!)}>{t("从不可变 Snapshot 重新分析", "Reanalyze immutable Snapshot")}</button></div>}
