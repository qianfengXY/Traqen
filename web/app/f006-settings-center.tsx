...
export function F006SettingsCenter(props: Props) {
  ...
  async function addGlobalCapability(kind: "SKILL" | "MCP") {
    ...
    if (!normalizedName || (kind === "SKILL" && !skillVerified)) return;
    ...
    const created = await props.onSaveGlobalCapability({
      ...
      manifest: {
        ...
        signature: kind === "SKILL" ? "VERIFIED" : undefined,
      },
    });
  ...
  if (props.scope === "global") return <section className="f006-settings-center">
    ...
    <div className="f006-global-shell">
      ...
      <div className="f006-global-content">
        ...
        {globalPage === "skills" && <div className="f006-info-note"><label className="f006-check-row"><input type="checkbox" checked={skillVerified} onChange={(event) => setSkillVerified(event.currentTarget.checked)} /><span>{props.t("我已验证此 Skill 的身份和行为", "I verified this Skill's identity and behavior")}</span><small>{props.t("验证后才能保存并授予 Agent", "Required before it can be saved and granted to an Agent")}</small></label></div>}
        {(globalPage === "skills" || globalPage === "mcp") && <GlobalCapabilities {...props} createDisabled={props.working || (globalPage === "skills" && !skillVerified)} kind={globalPage === "skills" ? "SKILL" : "MCP"} name={capabilityName} setName={setCapabilityName} description={capabilityDescription} setDescription={setCapabilityDescription} onAdd={() => void addGlobalCapability(globalPage === "skills" ? "SKILL" : "MCP")} onLifecycle={(target) => void requestLifecycle(target)} />}
      </div>
    </div>
  ...
  return <section className="f006-settings-center">
    ...
    {workspacePage === "capabilities" && <WorkspaceCapabilities {...props} localKind={localKind} setLocalKind={setLocalKind} localName={localName} setLocalName={setLocalName} onToggle={toggleWorkspaceCapability} onOpenAgentSettings={(agentId) => { setWorkspacePage("agents"); setSelectedAgentId(agentId); setAgentDrawerOpen(true); }} onAddLocal={() => { const normalizedName = localName.trim().toLowerCase(); if (!normalizedName) return; props.onSaveLocalCapability({ kind: localKind, normalizedName, expectedVersion: 0, manifest: { description: `${props.workspace?.name ?? "Workspace"} local capability`, signature: localKind === "SKILL" ? "VERIFIED" : undefined } }); setLocalName(""); }} />}
  </section>;
}
...
function GlobalCapabilities(props: Props & { createDisabled: boolean; kind: "SKILL" | "MCP"; name: string; setName: (value: string) => void; description: string; setDescription: (value: string) => void; onAdd: () => void; onLifecycle: (target: LifecycleTarget) => void }) {
