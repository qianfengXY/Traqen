...
type Props = {
  ...
  executableSkills: WorkspaceExecutableSkill[];
  ...
  onAutoSave: (editRevision: number) => Promise<boolean>;
  onApply: () => void;
  onRetryDraftConflict: () => Promise<number | null>;
...
const OAUTH_ADAPTERS = ["CODEX", "CLAUDE"] as const;

function typedKey(kind: "SKILL" | "MCP", normalizedName: string) {
  return `${kind}:${normalizedName}`;
}

function executableSkillKey(skill: WorkspaceExecutableSkill) {
...
function isEnvironmentSecretReference(value: string) {
  return /^env:\/\/[A-Z_][A-Z0-9_]*$/.test(value.trim());
}
...
export function F006SettingsCenter(props: Props) {
  const { onAutoSave, recoveryReady, working } = props;
  ...
  const [globalSkillKey, setGlobalSkillKey] = useState("");
  const [localName, setLocalName] = useState("");
  const [localKind, setLocalKind] = useState<"SKILL" | "MCP">("SKILL");
  const [localSkillKey, setLocalSkillKey] = useState("");
  ...
  const availableModels = useMemo(() => props.models.filter((model) => {
    const account = props.accounts.find((candidate) => candidate.accountId === model.accountId);
  ...
  const unavailableCapabilities = useMemo(() => props.catalog.entries.filter((entry) => entry.availability === "GLOBAL_UNAVAILABLE"), [props.catalog.entries]);
  const selectedGlobalSkill = props.executableSkills.find((skill) => executableSkillKey(skill) === globalSkillKey) ?? null;
  const selectedLocalSkill = props.executableSkills.find((skill) => executableSkillKey(skill) === localSkillKey) ?? null;
  ...
  useEffect(() => {
    if (!edited || edited === savedEdited || !recoveryReady || working || autosaveStatus === "ERROR" || props.draftConflict || autosaveInFlight.current) return;
    const editRevision = edited;
    const timer = window.setTimeout(() => {
      autosaveInFlight.current = true;
      setAutosaveStatus("SAVING");
      void onAutoSave(editRevision).then((saved) => {
        if (saved) {
          setSavedEdited((current) => Math.max(current, editRevision));
          setAutosaveStatus("SAVED");
        } else {
          setAutosaveStatus("ERROR");
        }
      }).catch(() => setAutosaveStatus("ERROR")).finally(() => { autosaveInFlight.current = false; });
    }, 650);
    return () => window.clearTimeout(timer);
  }, [autosaveAttempt, autosaveStatus, edited, onAutoSave, props.draftConflict, recoveryReady, savedEdited, working]);
  ...
  async function addGlobalCapability(kind: "SKILL" | "MCP") {
    ...
    if (!normalizedName || (kind === "SKILL" && (!skillVerified || !selectedGlobalSkill))) return;
    ...
    const created = await props.onSaveGlobalCapability({
      ...
      lifecycle: "ACTIVE",
      manifest: {
        ...(capabilityDescription.trim() ? { description: capabilityDescription.trim() } : {}),
        ...(kind === "SKILL" ? { adapterId: selectedGlobalSkill!.id, version: selectedGlobalSkill!.version } : {}),
      },
    });
    if (created) { setCapabilityName(""); setCapabilityDescription(""); setSkillVerified(false); }
  }
  ...
  async function confirmLifecycle() {
    ...
    const requiresTypedConfirmation = lifecycleTarget.lifecycle === "DELETED" || Boolean(impact?.impacts.length);
  ...
  if (props.scope === "global") return <section className="f006-settings-center">
    ...
    <div className="f006-global-shell">
      <nav className="f006-settings-nav" aria-label={props.t("全局设置导航", "Global settings navigation")}>
      ...
      <div className="f006-global-content">
        ...
        {globalPage === "skills" && <div className="f006-info-note"><label className="f006-check-row"><input type="checkbox" checked={skillVerified} onChange={(event) => setSkillVerified(event.currentTarget.checked)} /><span>{props.t("我已确认此 Skill 使用下方已挂载执行器", "I confirmed this Skill uses the mounted executor selected below")}</span><small>{props.t("服务端会验证执行器映射后再签署并授予 Agent。", "The server verifies the executor mapping before sealing it for Agent grants.")}</small></label></div>}
        {(globalPage === "skills" || globalPage === "mcp") && <GlobalCapabilities {...props} createDisabled={props.working || (globalPage === "skills" && (!skillVerified || !selectedGlobalSkill))} kind={globalPage === "skills" ? "SKILL" : "MCP"} name={capabilityName} setName={setCapabilityName} description={capabilityDescription} setDescription={setCapabilityDescription} skillKey={globalSkillKey} setSkillKey={setGlobalSkillKey} onAdd={() => void addGlobalCapability(globalPage === "skills" ? "SKILL" : "MCP")} onLifecycle={(target) => void requestLifecycle(target)} />}
      </div>
    </div>
  ...
  return <section className="f006-settings-center">
    ...
    {props.draftConflict && <div className="f006-draft-conflict" role="alert"><div><b>{props.t("草稿已在其他位置更新", "The draft changed elsewhere")}</b><span>{props.t("本地编辑仍被保留。先选择重试自己的版本，或采用服务器的新版本。", "Your edits are retained. Retry your version or adopt the newer server draft.")}</span></div><div><button className="button" disabled={props.working} onClick={() => { void props.onRetryDraftConflict().then(acknowledgeRecoveredDraft); }}>{props.t("重试我的草稿", "Retry my draft")}</button><button className="button" disabled={props.working} onClick={() => { void props.onUseCurrentDraft(edited).then(acknowledgeRecoveredDraft); }}>{props.t("采用服务器草稿", "Use server draft")}</button></div></div>}
    ...
    {workspacePage === "capabilities" && <WorkspaceCapabilities {...props} localKind={localKind} setLocalKind={setLocalKind} localName={localName} setLocalName={setLocalName} skillKey={localSkillKey} setSkillKey={setLocalSkillKey} onToggle={toggleWorkspaceCapability} onOpenAgentSettings={(agentId) => { setWorkspacePage("agents"); setSelectedAgentId(agentId); setAgentDrawerOpen(true); }} onAddLocal={() => { const normalizedName = localName.trim().toLowerCase(); if (!normalizedName || (localKind === "SKILL" && !selectedLocalSkill)) return; props.onSaveLocalCapability({ kind: localKind, normalizedName, expectedVersion: 0, manifest: { description: `${props.workspace?.name ?? "Workspace"} local capability`, ...(localKind === "SKILL" ? { adapterId: selectedLocalSkill!.id, version: selectedLocalSkill!.version } : {}) } }); setLocalName(""); }} />}
  </section>;
}
...
function GlobalCapabilities(props: Props & { createDisabled: boolean; kind: "SKILL" | "MCP"; name: string; setName: (value: string) => void; description: string; setDescription: (value: string) => void; skillKey: string; setSkillKey: (value: string) => void; onAdd: () => void; onLifecycle: (target: LifecycleTarget) => void }) {
  const entries = props.capabilities.filter((entry) => entry.kind === props.kind);
  return <div className="f006-page-grid"><section className="f006-form-card"><div><p className="eyebrow">{props.kind}</p><h2>{props.t(`添加全局 ${props.kind}`, `Add global ${props.kind}`)}</h2><p>{props.t("全局启用项会进入项目可选目录，但绝不自动授予 Agent。", "A global active item enters Workspace availability, but is never auto-granted to an Agent.")}</p></div><label>{props.t("名称", "Name")}<input value={props.name} onChange={(event) => props.setName(event.currentTarget.value)} placeholder={props.kind === "SKILL" ? "review" : "repository-tools"} /></label>{props.kind === "SKILL" && <label>{props.t("已挂载执行器", "Mounted executor")}<select value={props.skillKey} onChange={(event) => props.setSkillKey(event.currentTarget.value)}><option value="">{props.t("选择服务器已挂载的执行器", "Select a server-mounted executor")}</option>{props.executableSkills.map((skill) => <option key={executableSkillKey(skill)} value={executableSkillKey(skill)}>{skill.displayName} · {skill.id}@{skill.version}</option>)}</select><small>{props.t("只有服务器实际挂载的执行器可以保存；服务端会封存其精确版本。", "Only executors actually mounted by the server can be saved; the server seals the exact version.")}</small></label>}<label>{props.t("说明（可选）", "Description (optional)")}<textarea value={props.description} onChange={(event) => props.setDescription(event.currentTarget.value)} rows={4} placeholder={props.t("给其他项目管理员看的说明", "A note for administrators in other Workspaces")} /></label><button className="button primary" disabled={props.createDisabled || !props.name.trim()} onClick={props.onAdd}>{props.t("添加并启用", "Add and enable")}</button></section><section className="f006-list-card"><div className="f006-list-head"><div><p className="eyebrow">Global {props.kind}</p><h2>{props.t("能力库", "Capability library")}</h2></div><span>{entries.length}</span></div>{entries.length ? entries.map((entry) => <article className="f006-resource-row" key={entry.id}><div className="f006-resource-icon">{props.kind === "SKILL" ? "✦" : "↔"}</div><div><strong>{entry.normalizedName}</strong><small>{String(entry.manifest.description ?? props.t("没有说明", "No description"))}</small><p>{entry.lifecycle === "ACTIVE" ? props.t("可被项目继承", "Available to Workspaces") : entry.lifecycle === "INACTIVE" ? props.t("项目不能重新启用", "Workspaces cannot re-enable it") : props.t("已删除；保留历史证据", "Deleted; historical evidence remains")}</p></div><div className={`f006-status ${entry.lifecycle === "ACTIVE" ? "success" : entry.lifecycle === "DELETED" ? "danger" : "warning"}`}>{entry.lifecycle}</div>{entry.lifecycle === "ACTIVE" && <button className="button" disabled={props.working} onClick={() => props.onLifecycle({ capability: entry, lifecycle: "INACTIVE" })}>{props.t("全局停用", "Deactivate")}</button>}{entry.lifecycle === "INACTIVE" && <button className="button" disabled={props.working} onClick={() => void props.onSetCapabilityLifecycle(entry.kind, entry.normalizedName, { expectedVersion: entry.revision, lifecycle: "ACTIVE" })}>{props.t("重新启用", "Reactivate")}</button>}{entry.lifecycle !== "DELETED" && <button className="button danger" disabled={props.working} onClick={() => props.onLifecycle({ capability: entry, lifecycle: "DELETED" })}>{props.t("删除", "Delete")}</button>}</article>) : <EmptyList title={props.t(`还没有 ${props.kind}`, `No ${props.kind}s yet`)} detail={props.t("添加一个全局能力，供 Workspace 按需选择。", "Add a global capability for Workspaces to choose deliberately.")} />}</section></div>;
}

function AgentSettings(props: Props & { selected: { id: string; title: string; model: string; skills: string[]; mcps: string[] } | null; selectedAgentId: string; setSelectedAgentId: (value: string) => void; agentDrawerOpen: boolean; setAgentDrawerOpen: (open: boolean) => void; availableModels: GlobalModelProfile[]; effectiveCapabilities: EffectiveCapability[]; unavailableCapabilities: EffectiveCapability[]; onModelChange: (model: string) => void; onToggleGrant: (kind: "SKILL" | "MCP", name: string) => void; onAddChild: () => void; onRemoveChild: (id: string) => void }) {
  const selectAgent = (id: string) => { props.setSelectedAgentId(id); props.setAgentDrawerOpen(true); };
  const unavailableGrants = props.selected
    ? props.unavailableCapabilities.filter((entry) => entry.kind === "SKILL"
      ? props.selected!.skills.includes(entry.normalizedName)
      : props.selected!.mcps.includes(entry.normalizedName))
    : [];
  const availableSkills = [...props.effectiveCapabilities.filter((entry) => entry.kind === "SKILL"), ...unavailableGrants.filter((entry) => entry.kind === "SKILL")];
  const availableMcps = [...props.effectiveCapabilities.filter((entry) => entry.kind === "MCP"), ...unavailableGrants.filter((entry) => entry.kind === "MCP")];
  return <div className="f006-agent-layout"><aside className="f006-agent-rail"><p className="eyebrow">Team</p><button className={`f006-agent-card main ${props.selectedAgentId === "MAIN" ? "selected" : ""}`} onClick={() => selectAgent("MAIN")}><span>♛</span><strong>Main Agent</strong><small>{props.mainModel || props.t("待选择模型", "Model required")}</small><em>{props.mainSkillNames.length + props.mainMcpNames.length} {props.t("项授权", "grants")}</em></button>{props.childSlots.map((slot, index) => <div key={slot.id} className="f006-child-wrap"><button className={`f006-agent-card ${props.selectedAgentId === slot.id ? "selected" : ""}`} onClick={() => selectAgent(slot.id)}><span>◌</span><strong>{props.t(`Child ${index + 1}`, `Child ${index + 1}`)}</strong><small>{slot.model || props.t("待选择模型", "Model required")}</small><em>{slot.skillNames.length + slot.mcpNames.length} {props.t("项授权", "grants")}</em></button>{props.childSlots.length > 1 && <button className="f006-remove-child" aria-label={props.t("移除此 Child", "Remove Child")} onClick={() => props.onRemoveChild(slot.id)}>×</button>}</div>)}<button className="button f006-add-child" disabled={props.working} onClick={props.onAddChild}>＋ {props.t("添加 Child", "Add Child")}</button></aside><section className={`f006-agent-inspector ${props.agentDrawerOpen ? "mobile-open" : ""}`}><button className="button f006-mobile-agent-back" onClick={() => props.setAgentDrawerOpen(false)}>{props.t("返回团队", "Back to team")}</button>{props.selected ? <><div className="f006-inspector-head"><div><p className="eyebrow">Agent configuration</p><h2>{props.selected.title}</h2><p>{props.t("模型和授权都必须显式选择。", "Both model and grants must be selected explicitly.")}</p></div><span className={`f006-status ${props.selected.model ? "success" : "warning"}`}>{props.selected.model ? props.t("已配置", "Configured") : props.t("待配置", "Incomplete")}</span></div><label>{props.t("全局 CLI 模型", "Global CLI model")}<select value={props.selected.model} onChange={(event) => props.onModelChange(event.currentTarget.value)}><option value="">{props.t("选择已验证模型", "Select a verified model")}</option>{props.availableModels.map((model) => <option key={model.profileId} value={model.profileId}>{model.displayName} · {model.cliAdapter}</option>)}</select></label><div className="f006-grant-block"><div><h3>Skill</h3><small>{props.t("当前有效的能力可授予；不可用的现有授权保留在此，取消勾选即可移除。", "Effective capabilities can be granted; unavailable existing grants remain here so you can uncheck to remove them.")}</small></div>{availableSkills.length ? availableSkills.map((entry) => <label className="f006-check-row" key={entry.id}><input type="checkbox" checked={props.selected!.skills.includes(entry.normalizedName)} onChange={() => props.onToggleGrant("SKILL", entry.normalizedName)} /><span>{entry.normalizedName}</span><small>{capabilityLabel(entry)}</small></label>) : <p className="f006-empty-inline">{props.t("没有可授予的 Skill。", "No Skills can be granted.")}</p>}</div><div className="f006-grant-block"><div><h3>MCP</h3><small>{props.t("项目允许与 Agent 授权是两个独立层。", "Workspace availability and Agent grants are separate layers.")}</small></div>{availableMcps.length ? availableMcps.map((entry) => <label className="f006-check-row" key={entry.id}><input type="checkbox" checked={props.selected!.mcps.includes(entry.normalizedName)} onChange={() => props.onToggleGrant("MCP", entry.normalizedName)} /><span>{entry.normalizedName}</span><small>{capabilityLabel(entry)}</small></label>) : <p className="f006-empty-inline">{props.t("没有可授予的 MCP。", "No MCPs can be granted.")}</p>}</div></> : <EmptyList title={props.t("选择一个 Agent", "Select an Agent")} detail={props.t("从左侧团队中选择。", "Choose one from the team.")} />}</section></div>;
}

function WorkspaceCapabilities(props: Props & { localKind: "SKILL" | "MCP"; setLocalKind: (value: "SKILL" | "MCP") => void; localName: string; setLocalName: (value: string) => void; skillKey: string; setSkillKey: (value: string) => void; onToggle: (entry: EffectiveCapability) => void; onOpenAgentSettings: (agentId: string) => void; onAddLocal: () => void }) {
  const groups = [
...
function LifecycleDialog(props: { target: LifecycleTarget; impact: GlobalCapabilityImpact | null; confirmation: string; setConfirmation: (value: string) => void; onCancel: () => void; onConfirm: () => void }) {
  const needsConfirmation = props.target.lifecycle === "DELETED" || Boolean(props.impact?.impacts.length);
  const action = props.target.lifecycle === "DELETED" ? "删除" : "全局停用";
  return <div className="modal-backdrop"><section className="confirmation-modal f006-lifecycle-dialog" role="dialog" aria-modal="true"><p className="eyebrow">Global impact</p><h2>{action} {props.target.capability.normalizedName}</h2><p>{props.impact?.impacts.length ? `此操作会影响 ${props.impact.impacts.length} 个活动 Workspace 配置。已在运行的任务仍使用其启动快照；新的运行只有在实际授予此能力时才会被阻断。` : "没有活动 Workspace 配置授予此能力。项目目录中的未授权项不会阻断运行。"}</p>{props.impact?.impacts.length ? <ul>{props.impact.impacts.map((item) => <li key={item.workspaceId}>{item.workspaceName ?? item.workspaceId} · {item.grantedSlotIds.join(", ")}</li>)}</ul> : null}{needsConfirmation && <label>输入 <b>{props.target.capability.normalizedName}</b> 确认<input value={props.confirmation} onChange={(event) => props.setConfirmation(event.currentTarget.value)} autoComplete="off" /></label>}<div className="modal-actions"><button className="button" onClick={props.onCancel}>取消</button><button className="button danger" disabled={needsConfirmation && props.confirmation !== props.target.capability.normalizedName} onClick={props.onConfirm}>{action}</button></div></section></div>;
}
