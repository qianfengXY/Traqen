...
export function sourceJourney(run: SourceRunState | null, draftRevision: number, confirmation: boolean, selected: number | null = null) {
  ...
  const state = run ? states[run.status] ?? { tone: "info", label: "服务端处理中", action: "QUERY_RESULT" } : { tone: "info", label: draftRevision ? "草稿已保存 · 待开始" : "开始建立来源快照", action: draftRevision ? "START" : "SAVE" };
