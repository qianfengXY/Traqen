...
export type SourceRun = { id: string; workspaceId: string; actorId: string; status: string; station: number; input: DraftInput; draftRevision: number; progress: { entries?: string; knownBytes?: string; pendingCount?: string; waitingFor?: string; candidate?: Candidate }; diagnostic: { code: string; message: string; recovery: string } | null; createdAt: string; updatedAt: string };
...
export type Confirmation = { id: string; actorId: string; candidateId: string; gapSetId: string; reason: string | null; expiresAt: string | null; confirmedAt: string };
...
export type FrozenVersion = { id: string; publishedAt: string; counts: Counts; identity: { inventoryId: string; gapSetId: string }; components: (Counts & { id: string; sourceId: string; kind: string; scope: SourceInput["scope"]; nativeIdentity: SourceSummary["nativeIdentity"] })[]; latestReceipt: Receipt | null; currentAdmission: "NOT_CHECKED" };
