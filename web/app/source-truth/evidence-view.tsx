...
export function ArtifactTable({ client, route }: { client: SourceTruthClient; route: string }) {
  ...
  return <div className="st-evidence"><h3>材料清单</h3><p className="st-muted">分页展示所有条目与处置。计数包含目录和不可用项，不等于完整可分析文件数。</p>
    {error ? <p role="alert" className="st-callout danger">{error}</p> : !page ? <p role="status">正在读取清单…</p> : <>
      <div className="st-table-scroll"><table><thead><tr><th>路径</th><th>类型</th><th>字节</th><th>处置</th></tr></thead><tbody>{page.items.map(({ entry, disposition, componentId }) => <tr key={`${componentId ?? ""}:${entry.pathBytes}`}><td title={entry.pathBytes}>{sourcePath(entry.pathBytes)}</td><td>{entry.kind}</td><td>{entry.sizeBytes ?? "—"}</td><td><span className={`st-badge ${disposition?.disposition === "EXTERNAL_GAP" ? "warning" : "muted"}`}>{disposition?.disposition ?? "待采集"}</span>{disposition?.reasonCode && <small>{disposition.reasonCode}</small>}</td></tr>)}</tbody></table></div>
