...
function SourceWorkspaceSession({ apiBase, token, workspaceId, workspaceName, onDirtyChange }: { apiBase: string; token: string; workspaceId: string; workspaceName: string; onDirtyChange?: (dirty: boolean) => void }) {
  ...
  const [latestVersion, setLatestVersion] = useState<FrozenVersion | null>(null);
  ...
  const refresh = useCallback(async () => {
    ...
    if (!historyCursor.current.bundles) setLatestVersion(versionPage.items[0] ?? null);
  ...
  return <>
    ...
    {!overview ? <SourceAccessState error={error} /> : <>
      ...
      <section id="source-version-history" className="st-panel st-history"><div className="st-card-head"><div><h2>版本与任务历史</h2><p className="st-muted">失败、取消与旧包仍可追溯；新任务失败不替换旧基线。</p></div>{writable && !overview.activeRun && <button className="button" disabled={busy || !frozen} onClick={() => newDraft(frozen)}>从选中的冻结包创建新版本</button>}</div>
