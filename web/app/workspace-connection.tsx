...
export function WorkspaceConnection({ heading, description, t, checking, issue, apiBase, token, onToken, onConnect, onDiagnostics }: {
  heading?: string; description?: string; t: T; checking: boolean; issue: ConnectionIssue | null; apiBase: string; token: string;
  ...
  return <section className="workspace-connection panel" aria-labelledby="workspace-connection-title" aria-busy={checking}>
    <header><p className="eyebrow">Traqen</p><h1 id="workspace-connection-title">{heading ?? t("连接工作空间", "Connect to your workspace")}</h1>
      <p>{description}</p><p>{t("先验证访问身份，再选择或创建 Workspace。连接失败不代表工作空间为空。", "Verify access before selecting or creating a Workspace. A failed connection does not mean it is empty.")}</p></header>
