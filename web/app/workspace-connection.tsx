...
export function WorkspaceConnection({ t, checking, issue, apiBase, token, onToken, onConnect, onDiagnostics }: {
  ...
  return <section className="workspace-connection panel" aria-labelledby="workspace-connection-title" aria-busy={checking}>
    ...
    {checking ? <p role="status">{t("正在验证连接与访问权限…", "Checking connection and access…")}</p> : <>
      ...
      <form onSubmit={(event) => { event.preventDefault(); onConnect(); }}>
        ...
        <div className="connection-actions"><button className="button primary" type="submit">{t("验证并连接", "Verify and connect")}</button><button className="button" type="button" onClick={onDiagnostics}>{t("连接设置", "Connection settings")}</button></div>
      </form>
    </>}
