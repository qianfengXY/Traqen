...
- Local Workspace analysis lets a user name a Workspace, choose a source directory without uploading it, and initialize the product-level session from supported files in page memory. The resulting Feature tree, selected Feature, trace block, and expansion state survive navigation; Feature traceability reuses that tree so candidates can be reviewed one by one without rescanning. A full browser refresh still clears local source-derived data.
