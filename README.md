...
Create and switch the aggregate root through `/v1/workspaces`. For a server-owned source run, configure `TRAQEN_ALLOWED_WORKSPACE_ROOTS`, `SOURCE_SNAPSHOT_ROOT`, and a high-entropy `SOURCE_SLICE_WORKER_CREDENTIAL_SECRET`; register an allowlisted root through `/source-registrations`, then start `/workspace-analysis-jobs`. SourceSlice HTTP reads require both normal API authentication and a short-lived, single-use worker credential issued for the exact project, Snapshot, run, WorkUnit, selected producer, and policy. A caller-supplied service identity is never trusted. The job persists the complete inventory, immutable Snapshot, static Facts, a pinned `WorkspaceExecutionProfileRevision`, identical sealed batches for one or more Child slots (default two), isolated Child results, reconciliation ledgers, reviewed evaluation, graph revision, and atomic current head. Production publication is fail-closed: configure `UNDERSTANDING_TRUTH_SET_PATH`, `UNDERSTANDING_TRUTH_SET_REVIEWER_ID`, `UNDERSTANDING_REVIEWED_MEASUREMENTS_PATH`, and `UNDERSTANDING_EQUIVALENCE_EVIDENCE_PATH`. The reviewed-measurement file supplies independently approved observations bound to the exact production input; the equivalence file supplies independently executed replay and FULL semantic surfaces. Missing, stale, self-reviewed, tampered, or non-equivalent evidence cannot move `CurrentGraphHead`. The older `/analysis-runs` and browser-observation import remain compatibility inputs only; they do not create governed truth. See [F001](docs/features/F001-legacy-system-understanding.md) and [F006](docs/features/F006-workspace-capability-settings.md).
