...
Create and switch the aggregate root through `/v1/workspaces`. For a server-owned source run, configure `TRAQEN_ALLOWED_WORKSPACE_ROOTS` and `SOURCE_SNAPSHOT_ROOT`, register an allowlisted root through `/source-registrations`, then start `/workspace-analysis-jobs`. The job persists the complete inventory, immutable Snapshot, static Facts, a pinned `WorkspaceExecutionProfileRevision`, identical sealed batches for one or more Child slots (default two), isolated Child results, reconciliation ledgers, reviewed evaluation, graph revision, and atomic current head. Configure reviewed Traqen-on-Traqen acceptance with `UNDERSTANDING_TRUTH_SET_PATH` and `UNDERSTANDING_TRUTH_SET_REVIEWER_ID`. The older `/analysis-runs` and browser-observation import remain compatibility inputs only; they do not create governed truth. See [F001](docs/features/F001-legacy-system-understanding.md) and [F006](docs/features/F006-workspace-capability-settings.md).
