...
The checked-in implementation already contains useful domain, scan, graph, history, impact, and model-profile pieces, but it does not yet implement the complete target product. In particular, the current UI's fixed Child panels and legacy scanner-/Fact-driven paths are implementation baselines, not the target contract. The approved redesign first fixes an immutable Workspace source boundary, then derives deterministic facts and the API tree, admits only human-reviewed Agent candidates into the business-function tree, and finally connects execution evidence to advisory impact and revalidation. See the [product architecture](docs/architecture/traqen-product-architecture.md), [F001–F004 feature specifications](docs/ROADMAP.md), and [documentation truth map](docs/README.md).
