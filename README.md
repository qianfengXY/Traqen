...
The checked-in implementation already contains useful domain, scan, graph, history, impact, and model-profile pieces, but it does not yet implement the complete target product. In particular, the current UI's fixed three Child panels and the legacy scanner-/Fact-driven paths are implementation baselines, not the target contract. The approved design makes Workspace the aggregate root, defaults to two configurable Child Agents, sends the same bounded batch to every active Child, lets the Main Agent reconcile all completed sibling results against static Facts, and resolves model/Skill/MCP access into an immutable Workspace-only execution profile. See the [product architecture](docs/architecture/traqen-product-architecture.md), [F001 lifecycle](docs/features/workspace-scan-and-analysis-lifecycle.md), and [documentation truth map](docs/README.md).
...
Create and switch the aggregate root through `/v1/workspaces`. For a server-owned source run, configure `TRAQEN_ALLOWED_WORKSPACE_ROOTS`, `SOURCE_SNAPSHOT_ROOT`, and a high-entropy `SOURCE_SLICE_WORKER_CREDENTIAL_SECRET`; register an allowlisted root through `/source-registrations`, then start `/workspace-analysis-jobs`. SourceSlice HTTP reads require both normal API authentication and a short-lived, single-use worker credential issued for the exact project, Snapshot, run, WorkUnit, selected producer, and policy. A caller-supplied service identity is never trusted. The job persists the complete inventory, immutable Snapshot, static Facts, a pinned `WorkspaceExecutionProfileRevision`, identical sealed batches for one or more Child slots (default two), isolated Child results, a pinned Main reconciliation result, reconciliation ledgers, reviewed evaluation, graph revision, and atomic current head. Production publication is fail-closed: configure `UNDERSTANDING_TRUTH_SET_PATH`, `UNDERSTANDING_TRUTH_SET_REVIEWER_ID`, `UNDERSTANDING_REVIEWED_MEASUREMENTS_PATH`, and `UNDERSTANDING_EQUIVALENCE_EVIDENCE_PATH`. Reviewed measurements identify every assessed Anchor, Candidate, relationship, and Gap and bind them to the exact persisted semantic-output digest, production input, Snapshot, run, Truth Set, and independent reviewer. Equivalence evidence identifies persisted terminal replay and FULL runs, their immutable semantic-surface records, pinned policy/profile, and distinct implementation author and runner; the server reloads and hashes both surfaces before comparing them. Missing, stale, self-reviewed, nonexistent, tampered, or non-equivalent evidence cannot move `CurrentGraphHead`. The Web product now invokes only the server-owned source-registration, workspace-analysis-job, and current-graph paths; older local scanner/model modules remain non-authoritative compatibility code and are not imported into the product entry. See [F001](docs/features/F001-legacy-system-understanding.md) and [F006](docs/features/F006-workspace-capability-settings.md).
