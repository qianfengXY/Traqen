...
本页将 co-creator 已确认的《F003 功能全景图 V1.0（第一版终稿）》及其功能说明写入设计文档。它是后续功能细化的基线，不代表 F003 已实现、技术接口已全部确定或功能已验收。

- **图稿：** 连线精修版，保持 8 组功能、五档分流和既定连接关系。
- **确认：** 2026-09-09 06:29 UTC，co-creator 确认“此图作为F003功能全景第一版终稿”。
- **写入授权：** 2026-09-09 07:07 UTC，co-creator 要求“你先把这一版写到设计文档里面吧”。
- **版本规则：** 本版原图不重新生成或覆盖；后续改变功能或图示时另建版本。

## 2. 功能全景图

![F003 功能全景图 V1.0：F001 主分析材料、F002 参考、主子 Agent 调查、五档分流、例外人工审核与可追溯图谱](assets/f003-functional-panorama-v1.0.zh-CN.png)

[打开定稿原图](assets/f003-functional-panorama-v1.0.zh-CN.png)。图内仍保留生成时的“讨论示意”标识；本页记录的 co-creator 定稿决定确定其 V1.0 身份，不修改原图上的文字。
...
| 03 | 主子 Agent 调查 | 查看调查任务与进度。 | 主 Agent 规划、分派、回读核查；至少一个子 Agent 跨材料读原文、追关系、查条件、找反例，并返回证据。 |
...
| 07 | 图谱浏览与追溯 | 切换业务、实现、覆盖视图；搜索、筛选、展开关联和双向反查。 | 展示业务功能/规则与文档、API/代码、配置/数据、测试资产的联系；节点和边保留状态与依据；结构化关系供 F004 消费。 |
...
图上的两条入图通路分别是“05 自动收录 → 07”与“06 人工决定校验后落实 → 07”；两条调查回路分别是“05 自动补证 → 03”与“06 补证/回答 → 03”。跨线弧表示线路跨越，不表示共享节点。
...
| Kimi 对精修图的复核 | `0001788923474475-000083-4ca85833`：连线精修版通过，功能内容未改变。 |
| co-creator 定稿确认 | `0001788935356126-000087-3385a3f2` |
| co-creator 写入授权 | `0001788937652971-000112-fea57317` |
| 原始生成产物 | `exec-e651da11-3710-4f73-9ecc-d276d6645dbe.png` |
| 归档图片 SHA-256 | `380002bce7bded27588afcae8c1a3f86e1c620e0f7bdd57bd56f7a3cc344a28b` |

归档图片与获确认原图逐字节一致。此追溯记录属于图稿与设计写入，不是代码 Review、Feature 完成或部署验收记录。
