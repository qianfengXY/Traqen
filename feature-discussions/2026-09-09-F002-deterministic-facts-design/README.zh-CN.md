...
# F002 确定性事实层：功能设计（全景 v4 · F003 风格版）

## 1. 本版是什么

本页以经 Kimi 检查通过、按 2026-09-15 07:06 UTC 授权落盘的功能全景 v4 为内容基线。根据 co-creator 随后的风格对齐要求及 07:53 UTC“你放到文档里面呢，我在文档里面看”，现将 **v4 的 F003 风格对齐版设为当前展示图**。功能、连线逻辑与待定项不变，仅调整视觉样式。**图稿落盘不代表接口合同已定、F002 已实现或获得实施授权；文档仍为讨论稿。**

本轮原样嵌入已生成的风格对齐版，不再重绘；v1–v4 原图及生成记录完整保留。Kimi 的独立检查针对原 v4；风格版的生成与作者视觉核查另有记录，不冒充新增独立验收。没有修改 F002/F003 正式 Spec、ADR、生命周期、代码或其他 Feature 文档。原文读取接口仍见第 9 节待对齐项，不因换风格而选定直连或短时句柄方案。
...
### 3.1 当前图稿：功能全景 v4 · F003 风格版

![F002 功能全景 v4 F003 风格版：六组二十四项子功能、受控材料视图、F003 主导调查、双向取证及待定接口](assets/f002-functional-panorama-v4-f003-style.zh-CN.png)

[打开当前风格版大图](assets/f002-functional-panorama-v4-f003-style.zh-CN.png) · [风格版提示词、来源与校验记录](assets/f002-functional-panorama-v4-f003-style.prompt.md) · [对照 v4 原图](assets/f002-functional-panorama-v4.zh-CN.png)。中英文文档引用同一份中文图片；图内编号为说明样例，不是实际产品记录。

视觉采用 F003 当前 V2.0 主图的浅灰背景、白色圆角卡片、黑灰文字与蓝色重点；只借用风格，不导入 F003 的功能或流程。紫色请求、蓝色返回、青色参考输出和琥珀色待定虚线仍保持各自含义。

v4 保留六组 24 项子功能、五部分出口及双向取证回路，落实三项图面修正：

- 主框标题为“F002 · 确定性处理”；独立验证保留在质量约束及补提／纠正确认中，不新增第七组功能。
- 黄色待对齐框与顶部 F001 之间增加虚线，明确标注“读取接口待定，非已定直连”。虚线不是已经批准的物理读取路径。
- “查原文／找反例”步骤增加同色“接口待定”角标，保留“现行 ADR-0003 准入边界仍有效”。

图内“参照 F003 全景 V1.0”记录 v4 内容绘制时的参考基线，不宣称它是 F003 当前最新版本；[F003 当前设计](../../docs/design/F003-traceability-graph/README.md) 已演进至 V2.0。本次按“仅换风格”要求保留图中文字；V2.0 是本次的视觉参考，不借此修改 F003 或关闭跨功能待对齐项。

### 3.2 历史图稿（不再作为当前主图）

| 版本 | 图稿与记录 | 保留用途 |
|---|---|---|
| v1 | [功能总览原图](assets/f002-functional-overview-v1.zh-CN.png) | 早期对象／事实示例；部分措辞已有后续纠正。 |
| v2 | [二级功能原图](assets/f002-functional-breakdown-v2.zh-CN.png) | 六组 24 项子功能的早期展开；01–06 是功能编号，不是事实编号。 |
| v3 | [功能全景](assets/f002-functional-panorama-v3.zh-CN.png) · [生成记录](assets/f002-functional-panorama-v3.prompt.md) | 引入材料视图、F003 主次分工与双向取证；v4 对标题和待定接口提示作局部修正。 |
| v4 原图 | [功能全景原版](assets/f002-functional-panorama-v4.zh-CN.png) · [生成记录](assets/f002-functional-panorama-v4.prompt.md) | 经 Kimi 检查的内容基线；当前风格版只调整视觉，原版保留供对照。 |
...
F003 是重要的应用验证渠道，但不是 F002 唯一的验收者。[F003 当前功能设计](../../docs/design/F003-traceability-graph/README.md) 采用“自动收录 Agent 分析态 + 例外人工审核”；本稿不恢复旧版“所有候选都必须先人工批准”的规定。
...
| 首次归档授权 | `0001788939317249-000000-39316dac`：“先把现有F002的功能与图，按当前讨论写一版”。 |
| 取证回路决策 | `0001789004687536-000113-c9b9f8c7`：增加 F003 → F002 的“定向取证／疑似漏提反馈”及返回路径。 |
| v3 重绘与检查建议 | 重绘授权 `0001789005669469-000128-c60201b3`；宪宪的三项图面建议 `0001789008610746-000150-a4270c5a`。 |
| v4 修改授权 | `0001789442627373-000010-7aa2f5de`：“宪宪是让你重新修改图片，如果你认可就改，不认可则反馈问题。” |
| v4 独立图稿检查 | Kimi，`0001789455416701-000276-ad9634c8`，2026-09-15 06:56 UTC：三项修改全部落实，图检查通过；不是代码或接口合同验收。 |
| v4 原版设计文档落盘授权 | `0001789455996764-000287-b731c634`，2026-09-15 07:06 UTC：“请把这版落到设计文档里面吧”。 |
| 风格对齐授权 | `0001789456971433-000308-709a079d`，2026-09-15 07:22 UTC：内容、线条保持现有逻辑，仅换成 F003 图的风格。 |
| 风格版嵌入文档授权 | `0001789458792213-000336-9a7994b2`，2026-09-15 07:53 UTC：“你放到文档里面呢，我在文档里面看”。 |
| F003 版本参照 | 图中参考基线为 `e3716b6` 的 V1.0；当前设计已在 `7cd6a75` 采用 V2.0。保留图内历史参照，不把引用版本演进当作接口对齐完成。 |

各代图片按生成原件逐字节归档，本轮仅更新文档引用，没有重绘或覆盖；v3/v4 及风格版的提示词和生成来源随图片一同保存：
...
相关入口：[现有 F002 Spec](../../docs/features/F002-feature-api-traceability.zh-CN.md)、[F003 当前功能设计](../../docs/design/F003-traceability-graph/README.md)、[来源边界 ADR-0003](../../docs/decisions/ADR-0003-source-truth-boundary.zh-CN.md)。这些文档不因本次讨论归档自动同步或变更状态。
