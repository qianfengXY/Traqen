...
# F002 确定性事实层：功能设计（全景 v4）

## 1. 本版是什么

本页最初按 2026-09-09 的授权归档功能与图稿；现根据 co-creator 于 2026-09-15 07:06 UTC 的“请把这版落到设计文档里面吧”，将经 Kimi 检查通过的 **功能全景 v4 设为当前图稿基线**，同步功能说明与取证反馈回路。**图稿落盘不代表接口合同已定、F002 已实现或获得实施授权；文档仍为讨论稿。**

本轮原样引用已检查的 v4 图片，不再重绘；v1–v3 及 v3/v4 生成记录完整保留。没有修改 F002/F003 正式 Spec、ADR、生命周期、代码或其他 Feature 文档。原文读取接口仍见第 9 节待对齐项，不因采用 v4 而选定直连或短时句柄方案。
...
![F002 功能全景 v4：六组二十四项子功能、受控材料视图、F003 主导调查、双向取证及待定接口](assets/f002-functional-panorama-v4.zh-CN.png)

[打开 v4 高清图](assets/f002-functional-panorama-v4.zh-CN.png) · [查看 v4 提示词、来源与校验记录](assets/f002-functional-panorama-v4.prompt.md)。中英文文档引用同一份中文图片；图内编号为说明样例，不是实际产品记录。
...
图内“参照 F003 全景 V1.0”记录绘图时的参考基线，不宣称它是 F003 当前最新版本；[F003 当前设计](../2026-09-09-F003-traceability-graph-design/README.zh-CN.md) 已演进至 V2.0。本次保留已检查图像的原始标签，不借此修改 F003 或关闭跨功能待对齐项。
...
| 本轮设计文档落盘授权 | `0001789455996764-000287-b731c634`，2026-09-15 07:06 UTC：“请把这版落到设计文档里面吧”。 |
| F003 版本参照 | 图中参考基线为 `e3716b6` 的 V1.0；当前设计已在 `7cd6a75` 采用 V2.0。保留图内历史参照，不把引用版本演进当作接口对齐完成。 |

各代图片按生成原件逐字节归档，本轮没有重绘或覆盖；v3/v4 的提示词及生成来源随图片一同保存：
