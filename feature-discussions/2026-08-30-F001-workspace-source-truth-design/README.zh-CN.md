...
> 本轮设计仅维护中文；[英文旧版](README.md)保留作历史参考，不与本轮修订视为等价。设计已确认；2026-09-06 已获完整编码实现授权，当前进入实现阶段，尚未完成运行验收。见[实施计划](../../feature-specs/2026-09-06-f001-source-truth-implementation.md)。
...
本节按 co-creator「把效果图放在文档里面，我在文档里面进行查看」（消息 `0001789717182538-000861-7bec2f99`）归档并内嵌三张既有效果图。图片由 AI 生成，参考 F005 的瓷白/石墨视觉，功能语义以本文为准；**是示例数据的设计效果图，不是产品运行截图，也不改变已确认的功能规则。** 正文 §4、§10.1 中“本次不修改 HTML/PNG”的表述指 2026-09-06 那轮修订，不限制本次明确授权的图片补充。

三张图分别呈现首次建包、后续版本核对、冻结结果；不是同一次采集的连续截图，也不是完整八站逐屏稿。图内示例身份、数字、日期和装饰不新增产品契约；本组不同场景与主题不作为 14/27 英寸统一缩放、主题等价或 F001 完工的验收证据。

### 01 · 首次添加来源（瓷白）

![F001 首次添加来源：Git 与目录两张独立来源卡，当前为未验证草稿](assets/ux-2026-09-18/01-first-source-light.png)

Git 和目录是两个独立入口，至少一种即可。首次建包 r1 尚在草稿阶段，commit 待预检解析，不提前显示已验证文件数。创建后继版本从第 2 站配置范围开始，不以这张首次建包图替代。

### 02 · 核对清单与缺口（瓷白）

![F001 第七站核对与缺口：214 项中 212 项已验证、2 项明确缺失，仍有待接受 Gap，冻结按钮禁用](assets/ux-2026-09-18/02-review-gaps-light.png)

后续版本 r9 的 214 项材料中，212 项内容已验证、2 项明确缺失。文件待处置为零不等于 Gap 已接受；两项非阻断 LFS 限制中仍有一项待接受，需由当前用户记录理由及绝对有效期限，冻结按钮保持禁用。安全或完整性阻断不能接受绕过。

### 03 · 冻结结果与当前准入（石墨）

![F001 第八站冻结结果：已接受缺口仍为橙色，保留 Bundle、Receipt 与来源原生身份，不自动启动分析](assets/ux-2026-09-18/03-frozen-with-gaps-dark.png)

r9 冻结后仍诚实显示橙色 `READY_WITH_ACCEPTED_GAPS`；Bundle、Receipt、Git commit、目录 manifest 分别表达身份，r8 历史版本保留不变。备份覆盖仍未验证，后续使用须重验当前权限、内容和接受期限；冻结来源包不自动启动分析。

以上 PNG 可直接在本文查看，点击图片可放大；详细流转合同继续见 §10.1，验收边界见 §12。既有八站图稿和英文历史文档保持不变。
...
| Receipt 与 F002 交接 | 已发布包取得 `READY` / `READY_WITH_ACCEPTED_GAPS` Receipt；`BLOCKED` 留在任务诊断，不签发 Receipt。F002 只接收合格来源包/输入。 | 后续能力不能基于路径、branch、部分上传，或未经接受的限制，产出看似权威的结论。 |
...
本次反馈修订另有明确授权：co-creator 在逐项建议分析后回复“可以按你的判断进行处理”（`0001788705321540-000110-6b5b4cc6`）。据此细化下述身份、诊断、发布、备份和安全合同，不将同行建议全部照搬，也不扩大为实现、旧文档清理或导航修改授权。
...
预检 `BLOCK` 保存 `SourcePreflightReport` 并使 run 阻断；采集/最终化前的阻断保存同一 run 的阶段诊断。两者都保留候选、原因和恢复动作，**零个新 Bundle、零个 Receipt**，不另建“无 Bundle 的 Receipt”类型。历史页分别列出任务结果和冻结版本；已冻结包后来的准入拒绝是当前检查结果，不把旧 Receipt 改成 `BLOCKED`。
...
| `CAPTURING` | 采集与校验中 | 查看进度、最终化前取消 | 进入 F002、修改正在采集的来源 | 服务端处理已收到字节；区分本地仍需上传与服务端正在验证，不用“后台运行”掩盖客户端依赖。 |
...
以下 `B-01`–`B-13` 是本文件独立的验收索引，不依赖 A 的旧 AC，也不表示任何一项已实现。
...
上轮已收口八站输入/产出/门槛、单来源与组件沿用路径、页面返回及修改失效、四维状态、条件性旧包可用性，以及 `B-01`–`B-12` 独立验收索引。

本次经反馈核验及消息 `0001788705321540-000110-6b5b4cc6` 授权，进一步固定 §6.2 身份格式，分离阻断诊断与 Receipt，细化批量预写/原子发布、服务端幂等绑定、执行接管、备份覆盖证明和存储安全，并增加 B-13 空来源验收。基线仍由用户显式选择；Git 克隆策略留给有界采集实现与测量，不硬编码成产品要求。接受可校验的 Git 空树，但不扩展目录零文件输入；不按候选摘要生成唯一发布身份，不增加独立可编辑的备份覆盖表。本段记录产品决定，不是同行 review verdict 或实现验收记录。

**设计修订边界（历史）：** 上轮只完善文件 B；A 为过时 Spec，不能作为当前设计或验收的约束。英文、A、ADR、HTML/PNG 未随设计修订修改；图稿中尚未体现的生命周期细节以本文为准，不能声称图片已经更新。

**当前实现授权：** co-creator 于 2026-09-06 明确“没有异议了，进行编码实现阶段吧。你可以进行开发了，按照这个方案进行，完整实现后再通知我。”（`0001788707385838-000118-bc711ace`）。据此进入[完整实施计划](../../feature-specs/2026-09-06-f001-source-truth-implementation.md)，覆盖 B-01～B-13，不改产品边界或 F005 导航，不将编码授权解释为使用生产数据实验。实现状态与设计确认分开；文档检查、原型或局部测试通过均不等于功能验收完成。
