...
> 本轮设计仅维护中文；[英文旧版](README.md)保留作历史参考，不与本轮修订视为等价。2026-09-19 按 co-creator 最新要求，将全文的界面说明对齐下列三张 UX 效果图，重点重写 §9–10。既有功能设计与实现授权保留；本文是设计合同，不代替实现、部署或 F001 整体验收记录。见[实施计划](../../feature-specs/2026-09-06-f001-source-truth-implementation.md)。

**阅读顺序：** 先看三张效果图和 §1–2 的产品结果，再按 §10.1、§10.3、§10.4 对照图中的区域、字段、按钮和状态；未单独出图的中间站见 §10.2，恢复见 §10.5，安全与存储见 §7–8，验收见 §10.8 与 §12。本文即 co-creator 指定的“文件 B”，是当前 F001 设计真相源；旧 Feature Spec（文件 A）不能反向约束或回退本方案。
