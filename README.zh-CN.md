...
该参考分析器具有证据边界且结果确定，但它生成的审核与等价记录均标记为 `LOCAL_DEVELOPMENT_REFERENCE_ONLY`；这些记录不是生产证据，也不能通过环境变量启用。`npm run api:serve` 仍使用默认拒绝不完整证据的 PostgreSQL 生产引导，在发布前继续要求独立审核的测量与等价证据。
